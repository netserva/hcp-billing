...
function ausValidateIntegerValue($number, $min_value = 1, $max_value = 999999999)
{
    ...
    if (!is_float($number) && false !== filter_var($number, FILTER_VALIDATE_INT, ['options' => ['min_range' => $min_value, 'max_range' => $max_value]])) { //don't allow numbers like 1.0 to bypass validation
        $result = true;
    }
            ...
            if (!empty($content_array['headers']['notification_server_signature'])) {
                ...
                if (!empty($content_array['body'])) {
                    ...
                    if (ausValidateIntegerValue($fwrite)) {
                        ...
                        if (true === $zip_file->open("{$script_root_directory}/{$zip_file_name}")) {
                            $zip_file->extractTo($script_root_directory);
                            $zip_file->close();

                            if (AUS_DELETE_EXTRACTED == 'YES') {
                                $removed_files_total = ausDeleteFileDirectory($script_root_directory, [$zip_file_name]);
...
function ausFetchQuery($query_type = 'upgrade', $version_number = '')
{
  ...
  function ausDeleteFileDirectory($root_directory, $files_array = [])
  {
      ...
      if (!empty($root_directory) && is_dir($root_directory)) {
          ...
          $files_array = array_diff($files_array, ['.', '..', '']);
