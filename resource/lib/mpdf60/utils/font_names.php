...
foreach ($ff as $f) {
    $ret = [];
    $isTTC = false;
    if ('.ttc' == strtolower(substr($f, -4, 4)) || '.ttcf' == strtolower(substr($f, -5, 5))) {	// Mac ttcf
        $isTTC = true;
    ...
    } elseif ('.ttf' == strtolower(substr($f, -4, 4)) || '.otf' == strtolower(substr($f, -4, 4))) {
...
foreach ($tempfontdata as $fname => $v) {
    ...
    unset($tempfontdata[$fname]['sip'], $tempfontdata[$fname]['smp'], $tempfontdata[$fname]['pua'], $tempfontdata[$fname]['puaag'], $tempfontdata[$fname]['unAGlyphs']);
}
