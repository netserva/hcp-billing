...
echo 'Full File URL: '.$file.'<br />';
    ...
    if ('jpeg' == $type || 'jpg' == $type) {
        ...
        if ($j) {
            ...
            if ($unitSp > 0) {
                ...
                if (2 == $unitSp) {	// = dots per cm (if == 1 set as dpi)
    ...
    elseif ('png' == $type) {
        ...
        $gAMA = 0;
        $j = strpos($data, 'gAMA');
        if ($j && !str_contains($data, 'sRGB')) {	// sRGB colorspace - overrides gAMA
        ...
        if (str_contains($data, 'sRGB')) {
            echo 'sRGB colorspace - NOT supported at present'.'<br />';
        }
        if (str_contains($data, 'cHRM')) {
               ...
               // Alpha channel set (including using tRNS for Paletted images)
                if ($pngalpha) {
                    ...
                    if ($p && 'Indexed' == $colspace) {
                        ...
                        if (0x1F == ord(substr($str, 0, 1)) && 0x8B == ord(substr($str, 1, 1))) {
...
function file_get_contents_by_curl($url, &$data): void
{
    $timeout = 5;
    $ch = curl_init($url);
...
{
    $type = '';
    if ('JFIF' == substr($data, 6, 4) || 'Exif' == substr($data, 6, 4) || substr($data, 0, 2) == chr(255).chr(216)) { // 0xFF 0xD8
        $type = 'jpeg';
    } elseif ('GIF87a' == substr($data, 0, 6) || 'GIF89a' == substr($data, 0, 6)) {
        $type = 'gif';
