...
function print_graph($g, $pgwidth)
{
    ...
    if (isset($g['attr']['SPLINES']) && $g['attr']['SPLINES'] && 'xy' == $type) {
        $splines = true;
    ...
    if (isset($g['attr']['PERCENT']) && $g['attr']['PERCENT'] && 'xy' != $type && 'scatter' != $type) {
        $percent = true;
    ...
    if (isset($g['attr']['ANTIALIAS']) && ('' == $g['attr']['ANTIALIAS'] || 0 == $g['attr']['ANTIALIAS'])) {
        $antialias = false;
    } elseif (isset($g['attr']['ANTIALIAS']) && $g['attr']['ANTIALIAS'] > 0) {
        $antialias = true;
    } elseif ('line' == $type || 'radar' == $type) {
        $antialias = false;
    } else {
        $antialias = true;
    }
    ...
    if (isset($g['attr']['SERIES']) && 'rows' == strtolower($g['attr']['SERIES'])) {
        $dataseries = 'rows';
    } else {
        $dataseries = 'cols';
    }
    ...
    if (isset($g['attr']['DATA-ROW-BEGIN']) && ('0' === $g['attr']['DATA-ROW-BEGIN'] || $g['attr']['DATA-ROW-BEGIN'] > 0)) {
        $rowbegin = $g['attr']['DATA-ROW-BEGIN'];
    }

    if (isset($g['attr']['DATA-COL-BEGIN']) && ('0' === $g['attr']['DATA-COL-BEGIN'] || $g['attr']['DATA-COL-BEGIN'] > 0)) {
        $colbegin = $g['attr']['DATA-COL-BEGIN'];
    }

    if (isset($g['attr']['DATA-ROW-END']) && ('0' === $g['attr']['DATA-ROW-END'] || 0 != $g['attr']['DATA-ROW-END'])) {
        $rowend = $g['attr']['DATA-ROW-END'];
    }
    if (isset($g['attr']['DATA-COL-END']) && ('0' === $g['attr']['DATA-COL-END'] || 0 != $g['attr']['DATA-COL-END'])) {
    ...
    // PERCENT
    if ($percent && 'pie' != $type && 'pie3d' != $type) {
    ...
    if (count($data) > 0 && $type) {
        $figure_file = 'graph_cache/'.random_int(11111, 999999999).'.'.$img_type;
        if ($bandw) {
