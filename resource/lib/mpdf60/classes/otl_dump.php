...
class OTLdump
{
    ...
    public function getMetrics($file, $fontkey, $TTCfontID, $debug, $BMPonly, $kerninfo, $useOTL, $mode): void
    {
        // mPDF 5.7.1
        $this->mode = $mode;
    ...
    public function extractInfo($debug = false, $BMPonly = false, $kerninfo = false, $useOTL = 0): void
    {
        $this->panose = [];
        $this->sFamilyClass = 0;
            ...
            for ($c = 0; $c < $Lookup[$i]['SubtableCount']; ++$c) {
                ...
                if (1 == $Lookup[$i]['Type']) {
                    ...
                    for ($s = 0; $s < count($Lookup[$i]['Subtable'][$c]['subs']); ++$s) {
                        ...
                        if (2 == $level && !str_contains($coverage, $inputGlyphs[0])) {
                            continue;
                        }
...
//print_r($volt[(count($volt)-1)]); exit;
            ...
            for ($c = 0; $c < $Lookup[$luli]['SubtableCount']; ++$c) {
                ...
                if (1 == $Lookup[$luli]['Type']) {
                    ...
                    if (1 == $PosFormat) {
                        ...
                        for ($g = 0; $g < count($glyphs); ++$g) {
                            if (2 == $level && !str_contains($lcoverage, $glyphs[$g])) {
                                continue;
                            }
                ...
                elseif (2 == $Lookup[$luli]['Type']) {
                    ...
                    if (1 == $PosFormat) {
                        ...
                        $glyphs = $this->_getCoverage(); // Array of Hex Glyphs
                        for ($p = 0; $p < $PairSetCount; ++$p) {
                            if (2 == $level && !str_contains($lcoverage, $glyphs[$p])) {
                                continue;
                            }
                            ...
                            for ($pv = 0; $pv < $PairValueCount; ++$pv) {
                                ...
                                if ($ValueFormat2) {
                                    // If RTL pairs, GPOS declares a XPlacement e.g. -180 for an XAdvance of -180 to take
                                    // account of direction. mPDF does not need the XPlacement adjustment
                                    if ('RTL' == $dir && $Value2['XPlacement'] && $Value2['XAdvance']) {
                    ...
                    elseif (2 == $PosFormat) {
                        ...
                        for ($i = 0; $i < $Class1Count; ++$i) {
                            for ($j = 0; $j < $Class2Count; ++$j) {
                                ...
                                if ('RTL' == $dir && $Value1['XPlacement'] && $Value1['XAdvance']) {
                                    $Value1['XPlacement'] -= $Value1['XAdvance'];
                                }
                                if ($ValueFormat2) {
                                    if ('RTL' == $dir && $Value2['XPlacement'] && $Value2['XAdvance']) {
                                ...
                                for ($c1 = 0; $c1 < count($Class1[$i]); ++$c1) {
                                    ...
                                    if (2 == $level && !str_contains($lcoverage, $FirstGlyph)) {
                                        continue;
                                    }
                ...
                elseif (4 == $Lookup[$luli]['Type']) {
                    ...
                    for ($i = 0; $i < count($MarkGlyphs); ++$i) {
                        if (2 == $level && !str_contains($lcoverage, $MarkGlyphs[$i])) {
                            continue;
                        }
                        if (!$firstMark) {
                ...
                elseif (6 == $Lookup[$luli]['Type']) {
                    ...
                    for ($i = 0; $i < count($Mark1Glyphs); ++$i) {
                        if (2 == $level && !str_contains($lcoverage, $Mark1Glyphs[$i])) {
                            continue;
                        }
                        if (!$firstMark) {
    ...
    public function formatEntity($char, $allowjoining = false)
    {
        ...
        $x = '&#x'.$char.';';
        if (str_contains($this->GlyphClassMarks, $char)) {
            if (!$allowjoining) {
                $x = '&#x25cc;'.$x;
            }
        }
    ...
    public function formatEntityArr($arr)
    {
        ...
        foreach ($arr as $c) {
            $c = preg_replace('/^[0]/', '', $c);
            $x = '&#x'.$c.';';
            if (str_contains($this->GlyphClassMarks, $c)) {
                $x = '&#x25cc;'.$x;
            }
            $s[] = $x;
        }
