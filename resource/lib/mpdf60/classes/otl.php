...
class otl
{
    ...
    public function applyOTL($str, $useOTL)
    {
        ...
        foreach ($earr as $char) {
            ...
            if (str_contains($this->GlyphClassMarks, $charasstr)) {
                $OTLdata[$subchunk][$charctr]['group'] = 'M';
            } elseif (32 == $char || 12288 == $char) {
        ...
        for ($sch = 0; $sch <= $subchunk; ++$sch) {
            ...
            elseif (UCDN::SCRIPT_ARABIC == $scriptblock || UCDN::SCRIPT_SYRIAC == $scriptblock) {
                $this->shaper = 'A';
            } // ARABIC shaper
            elseif (UCDN::SCRIPT_NKO == $scriptblock || UCDN::SCRIPT_MANDAIC == $scriptblock) {
                $this->shaper = 'A';
            } // ARABIC shaper
            elseif (UCDN::SCRIPT_KHMER == $scriptblock) {
                $this->shaper = 'K';
            } // KHMER shaper
            elseif (UCDN::SCRIPT_THAI == $scriptblock) {
                $this->shaper = 'T';
            } // THAI shaper
            elseif (UCDN::SCRIPT_LAO == $scriptblock) {
                $this->shaper = 'L';
            } // LAO shaper
            elseif (UCDN::SCRIPT_SINHALA == $scriptblock) {
                $this->shaper = 'S';
            } // SINHALA shaper
            elseif (UCDN::SCRIPT_MYANMAR == $scriptblock) {
                $this->shaper = 'M';
            } // MYANMAR shaper
            elseif (UCDN::SCRIPT_NEW_TAI_LUE == $scriptblock) {
                $this->shaper = 'E';
            } // SEA South East Asian shaper
            elseif (UCDN::SCRIPT_CHAM == $scriptblock) {
                $this->shaper = 'E';
            }  // SEA South East Asian shaper
            elseif (UCDN::SCRIPT_TAI_THAM == $scriptblock) {
                $this->shaper = 'E';
            } // SEA South East Asian shaper
            else {
                $this->shaper = '';
            ...
            if (count($ScriptLang)) {
                [$GSUBscriptTag, $is_old_spec] = $this->_getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $this->shaper, $useOTL, 'GSUB');
                if ($this->mpdf->fontLanguageOverride && str_contains($ScriptLang[$GSUBscriptTag], $this->mpdf->fontLanguageOverride)) {
            ...
            if ($GSUBscriptTag && $GSUBlangsys && isset($ScriptLang[$GSUBscriptTag]) && str_contains($ScriptLang[$GSUBscriptTag], $GSUBlangsys)) {
            ...
            elseif (count($ScriptLang)) {
                [$GPOSscriptTag, $dummy] = $this->_getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $this->shaper, $useOTL, 'GPOS');
                if ($GPOSscriptTag && $this->mpdf->fontLanguageOverride && str_contains($ScriptLang[$GPOSscriptTag], $this->mpdf->fontLanguageOverride)) {
            ...
            if (isset($this->mpdf->overrideOTLsettings, $this->mpdf->overrideOTLsettings[$this->fontkey])) {
            ...
            if (!$GSUBscriptTag && !$GSUBlangsys && !$GPOSscriptTag && !$GPOSlangsys) {
                ...
                for ($i = 0; $i < count($this->OTLdata); ++$i) {
                    if (8204 == $this->OTLdata[$i]['uni'] || 8205 == $this->OTLdata[$i]['uni']) {
            ...
            if ('M' == $this->shaper && 'mym2' != $GSUBscriptTag) {
                $this->shaper = '';
            }

            $GSUBFeatures = ($this->mpdf->CurrentFont['GSUBFeatures'][$GSUBscriptTag][$GSUBlangsys] ?? false);
            $GPOSFeatures = ($this->mpdf->CurrentFont['GPOSFeatures'][$GPOSscriptTag][$GPOSlangsys] ?? false);
            ...
            if ($this->mpdf->useDictionaryLBR && ('K' == $this->shaper || 'T' == $this->shaper || 'L' == $this->shaper)) {
            ...
            elseif ($this->mpdf->useTibetanLBR && UCDN::SCRIPT_TIBETAN == $scriptblock) {
                // Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
            ...
            if (($useOTL & 0xFF) && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
                ...
                if ('A' == $this->shaper) {
                    ...
                    for ($i = 0; $i < count($this->OTLdata); ++$i) {
                        ...
                        elseif (0xFEB3 == $this->OTLdata[$i]['uni'] || 0xFEB4 == $this->OTLdata[$i]['uni'] || 0xFEBB == $this->OTLdata[$i]['uni'] || 0xFEBC == $this->OTLdata[$i]['uni']) {
                            $checkpos = $i + 1;
                            while (isset($this->OTLdata[$checkpos]) && str_contains($this->GlyphClassMarks, $this->OTLdata[$checkpos]['hex'])) {
                                ++$checkpos;
                            }
                        ...
                        elseif (0xFE94 == $this->OTLdata[$i]['uni'] || 0xFEA2 == $this->OTLdata[$i]['uni'] || 0xFEAA == $this->OTLdata[$i]['uni']) {
                        ...
                        // Connecting to previous character
                        // Position: Before the character
                        elseif (0xFE8E == $this->OTLdata[$i]['uni'] || 0xFEC2 == $this->OTLdata[$i]['uni'] || 0xFEDE == $this->OTLdata[$i]['uni'] || 0xFEDA == $this->OTLdata[$i]['uni'] || 0xFB93 == $this->OTLdata[$i]['uni']) {
                        ...
                        elseif (0xFEAE == $this->OTLdata[$i]['uni'] || 0xFEF2 == $this->OTLdata[$i]['uni'] || 0xFEF0 == $this->OTLdata[$i]['uni'] || 0xFEF4 == $this->OTLdata[$i]['uni'] || 0xFBE9 == $this->OTLdata[$i]['uni'] || 0xFBFD == $this->OTLdata[$i]['uni'] || 0xFBFF == $this->OTLdata[$i]['uni']
                        ) {
                            $checkpos = $i - 1;
                            while (isset($this->OTLdata[$checkpos]) && str_contains($this->GlyphClassMarks, $this->OTLdata[$checkpos]['hex'])) {
                                --$checkpos;
                            }
                            if (isset($this->OTLdata[$checkpos]) && 0xFE92 == $this->OTLdata[$checkpos]['uni']) {
                        ...
                        // Connecting to previous character
                        // Position: Before the character
                        elseif (0xFEEE == $this->OTLdata[$i]['uni'] || 0xFECA == $this->OTLdata[$i]['uni'] || 0xFED6 == $this->OTLdata[$i]['uni'] || 0xFED2 == $this->OTLdata[$i]['uni']) {
                        ...
                        if (!isset($this->OTLdata[$i]['GPOSinfo']['kashida'])) {
                            if (str_contains($this->GSUBdata[$this->GSUBfont]['finals'], $this->OTLdata[$i]['hex'])) { // ANY OTHER FINAL FORM
                                $this->OTLdata[$i]['GPOSinfo']['kashida'] = 2;
                            } elseif (str_contains('0FEAE 0FEF0 0FEF2', $this->OTLdata[$i]['hex'])) { // not already included in 5 above
                    ...
                    for ($ptr = 1; $ptr < count($this->OTLdata); ++$ptr) {
                        if (UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK == $this->OTLdata[$ptr]['general_category'] && 32 == $this->OTLdata[$ptr - 1]['uni']) {
                ...
                //===================================
                elseif ('I' == $this->shaper || 'K' == $this->shaper || 'S' == $this->shaper) {
                    ...
                    for ($ptr = 0; $ptr < count($this->OTLdata); ++$ptr) {
                        ...
                        if ('I' == $this->shaper) {
                            if (0x09AF == $char && isset($this->OTLdata[$ptr + 1]) && 0x09BC == $this->OTLdata[$ptr + 1]['uni']) {
                                $sub = 0x09DF;
                                $newinfo = [];
                                $newinfo[0] = [];
                    ...
                    //-----------------------------------------------------------------------------------
                    if ('I' == $this->shaper || 'S' == $this->shaper) {
                    ...
                    if ('K' == $this->shaper) {  // Features are applied one at a time, working through each codepoint
                ...
                //==============================
                     ...
                     */
                    if ('T' == $this->shaper || 'L' == $this->shaper) {
                        for ($ptr = 0; $ptr < count($this->OTLdata); ++$ptr) {
                            $char = $this->OTLdata[$ptr]['uni'];
                    ...
                    if (UCDN::SCRIPT_TIBETAN == $scriptblock) {
                        // =========================
                        // Reordering TIBETAN
                        // =========================
                        ...
                        for ($ptr = 0; $ptr < count($this->OTLdata) - 1; ++$ptr) {
                            if (INDIC::in_range($this->OTLdata[$ptr]['uni'], 0x0F20, 0x0F33) && 0x0F3F == $this->OTLdata[$ptr + 1]['uni']) {
                    ...
                    if (!(($this->mpdf->textvar & FC_SMALLCAPS) && UCDN::SCRIPT_LATIN == $scriptblock && !str_contains($useGSUBtags, 'smcp'))) {
            ...
            //=======================================================
            if (('K' == $this->shaper || 'T' == $this->shaper || 'L' == $this->shaper) || UCDN::SCRIPT_TIBETAN == $scriptblock) {
                ...
                for ($i = count($this->OTLdata) - 1; $i > 0; --$i) {
                    ...
                    } elseif (0x2E == $this->OTLdata[$i]['uni']) { // Word end if Full-stop.
                        array_splice($this->OTLdata, $i + 1, 0, $newinfo);
                        $this->_updateLigatureMarks($i, 1);
                    }
                }
            ...
            //=======================================================
            if ('I' == $this->shaper || 'S' == $this->shaper || 'A' == $this->shaper || 'K' == $this->shaper || 'M' == $this->shaper) {
                // Remove ZWJ and ZWNJ
                for ($i = 0; $i < count($this->OTLdata); ++$i) {
                    if (8204 == $this->OTLdata[$i]['uni'] || 8205 == $this->OTLdata[$i]['uni']) {
                        array_splice($this->OTLdata, $i, 1);
            ...
            if (($useOTL & 0xFF) && $GPOSscriptTag && $GPOSlangsys && $GPOSFeatures) {
                ...
                if (UCDN::SCRIPT_LATIN != $scriptblock) {
                    $tags .= ' kern';
                }
                ...
                if (count($this->Entry) || count($this->Exit)) {
                    ...
                    $incurs = false;
                    for ($i = (count($this->OTLdata) - 1); $i >= 0; --$i) {
                        if (isset($this->Entry[$i], $this->Entry[$i]['Y']) && 'RTL' == $this->Entry[$i]['dir']) {
                            $nextbase = $i - 1; // Set as next base ignoring marks (next base reading RTL in logical oder
                            while (isset($this->OTLdata[$nextbase]['hex']) && str_contains($this->GlyphClassMarks, $this->OTLdata[$nextbase]['hex'])) {
                                --$nextbase;
                            }
                            if (isset($this->Exit[$nextbase], $this->Exit[$nextbase]['Y'])) {
                                ...
                                if (isset($this->Exit[$i]['X'], $this->Entry[$nextbase]['X'])) {
                        ...
                        } elseif (str_contains($this->GlyphClassMarks, $this->OTLdata[$i]['hex'])) {
                            continue;
                        } // ignore Marks
                        else {
                            $incurs = false;
                    ...
                    $incurs = false;
                    for ($i = 0; $i < count($this->OTLdata); ++$i) {
                        if (isset($this->Exit[$i], $this->Exit[$i]['Y']) && 'LTR' == $this->Exit[$i]['dir']) {
                            $nextbase = $i + 1; // Set as next base ignoring marks
                            while (str_contains($this->GlyphClassMarks, $this->OTLdata[$nextbase]['hex'])) {
                                ++$nextbase;
                            }
                            if (isset($this->Entry[$nextbase], $this->Entry[$nextbase]['Y'])) {
    ...
    public function _applyTagSettings($tags, $Features, $omittags = '', $onlytags = false)
    {
        ...
        for ($i = 0; $i < count($m[0]); ++$i) {
            $t = $m[1][$i];
            // Is it a valid tag?
            if (isset($Features[$t]) && !str_contains($omittags, $t) && (!$onlytags || str_contains($tags, $t))) {
                $usetags .= ' '.$t;
            }
        }
    ...
    public function _applyGSUBrules($usetags, $scriptTag, $langsys): void
    {
    ...
    public function _applyGSUBrulesSingly($usetags, $scriptTag, $langsys): void
    {
    ...
    public function _applyGSUBrulesMyanmar($usetags, $scriptTag, $langsys): void
    {
    ...
    public function _applyGSUBrulesIndic($usetags, $scriptTag, $langsys, $is_old_spec): void
    {
            ...
            foreach ($LookupList as $lu => $tag) {
                ...
                while ($ptr < (count($this->OTLdata))) { // whilst there is another glyph ..0064
                    ...
                    foreach ($this->GSUBLookups[$lu]['Subtables'] as $c => $subtable_offset) {
                        // NB Coverage only looks at glyphs for position 1 (esp. 7.3 and 8.3)
                        if (isset($this->GSLuCoverage[$lu][$c][$currGID])) {
                            if (str_contains('rphf pref blwf half pstf cfar init', $usetag)) { // only apply when mask indicates
                                $mask = 0;
                        ...
                        elseif (_OTL_OLD_SPEC_COMPAT_1 && 4 == $Type && !$is_old_spec && str_contains('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D', $currGlyph)) {
                            // only apply when 'pref blwf pstf' tags, and when mask indicates
                            if (str_contains('pref blwf pstf', $usetag)) {
                                $mask = 0;
    ...
    public function _applyGSUBsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $level, $currentTag, $is_old_spec, $tagInt)
    {
        ...
        if (1 == $Type) {
            ...
            // Format 1:
            //===========
            if (1 == $SubstFormat) { // Calculated output glyph indices
            ...
            // Format 2:
            //===========
            elseif (2 == $SubstFormat) { // Specified output glyph indices
        ...
        if (4 == $Type) {
            ...
            for ($g = 0; $g < $LigCount; ++$g) {
                ...
                for ($l = 1; $l < $CompCount; ++$l) {
                    ...
                    while (isset($this->OTLdata[$spos]) && str_contains($ignore, $this->OTLdata[$spos]['hex'])) {
                        ++$spos;
                    }
    ...
    public function _updateLigatureMarks($pos, $n): void
    {
        if ($n > 0) {
    ...
    public function GSUBsubstitute($pos, $substitute, $Type, $GlyphPos = null)
    {
        ...
        if (2 == $Type) {
            for ($i = 0; $i < count($substitute); ++$i) {
                ...
                if (str_contains($this->GlyphClassMarks, $newOTLdata[$i]['hex'])) {
                    $gp = 'M';
                } elseif (32 == $uni) {
                    $gp = 'S';
                } else {
                ...
                if ('I' == $this->shaper || 'K' == $this->shaper || 'S' == $this->shaper) {
            ...
            if ('K' == $this->shaper || 'T' == $this->shaper || 'L' == $this->shaper) {
        ...
        if (4 == $Type) {
            ...
            for ($i = 0; $i < count($GlyphPos); ++$i) {
                ...
                if (str_contains($this->GlyphClassMarks, $unistr)) {
                    $contains_marks = true;
                } else {
            ...
            if ($contains_marks && !$contains_nonmarks) {
                ...
                for ($i = 0; $i < count($GlyphPos); ++$i) {
                    ...
                    $ic = 1;
                    while ((($i == count($GlyphPos) - 1) || (isset($GlyphPos[$i + 1]) && ($GlyphPos[$i] + $ic) < $GlyphPos[$i + 1])) && isset($this->OTLdata[($GlyphPos[$i] + $ic)]) && str_contains($this->GlyphClassMarks, $this->OTLdata[($GlyphPos[$i] + $ic)]['hex'])) {
                        $newComp = $currComp;
            ...
            if (str_contains($this->GlyphClassMarks, $this->unicode_hex($substitute))) {
                $gp = 'M';
            } elseif (32 == $substitute) {
                $gp = 'S';
            } else {
                $gp = 'C';
            }
            ...
            if (1 == $ka && isset($this->OTLdata[$pos]['form']) && 3 == $this->OTLdata[$pos]['form']) {
            ...
            for ($i = count($GlyphPos) - 1; $i > 0; --$i) {
                ...
                unset($this->assocLigs[$gpos], $this->assocMarks[$gpos]);
            }
    ...
    public function arabic_initialise(): void
    {
    ...
    public function arabic_shaper($usetags, $scriptTag): void
    {
        $chars = [];
        for ($i = 0; $i < count($this->OTLdata); ++$i) {
        ...
        for ($i = $max - 1; $i >= 0; --$i) {
            ...
            if ($prevChar && isset($this->arabTransparentJoin[$prevChar], $chars[$i - 2])) {
                $prevChar = hexdec($chars[$i - 2]);
                if ($prevChar && isset($this->arabTransparentJoin[$prevChar], $chars[$i - 3])) {
                    $prevChar = hexdec($chars[$i - 3]);
                    if ($prevChar && isset($this->arabTransparentJoin[$prevChar], $chars[$i - 4])) {
    ...
    public function get_arab_glyphs($char, $type, &$chars, $i, $scriptTag, $usetags)
    {
        ...
        if ((0 === $type && !str_contains($usetags, 'isol')) || (1 === $type && !str_contains($usetags, 'fina')) || (2 === $type && !str_contains($usetags, 'init')) || (3 === $type && !str_contains($usetags, 'medi'))) {
            return [$char, 0];
        }
        ...
        if ('syrc' == $scriptTag && '00710' == $char) {
            ...
            if (isset($chars[$n], $this->arabLeftJoining[hexdec($chars[$n])])) {
                ...
                elseif (!isset($chars[$i + 1]) || !preg_match('/[\x{0700}-\x{0745}]/u', code2utf(hexdec($chars[$i + 1])))) {
                    ...
                    if (str_contains('0715|0716|072A', $chars[$n]) && isset($this->arabGlyphs[$char][6])) {
                        $retk = 6;
                    }
        ...
        if (($type > 0 || 0 === $type) && isset($this->arabGlyphs[$char][$type])) {
            $retk = $type;
        } elseif (3 == $type && isset($this->arabGlyphs[$char][1])) { // if <medial> not defined, but <final>, return <final>
            $retk = 1;
        } elseif (2 == $type && isset($this->arabGlyphs[$char][0])) { // if <initial> not defined, but <isolated>, return <isolated>
            $retk = 0;
        }
        if (-1 != $retk) {
            $match = true;
            ...
            if (isset($this->arabGlyphs[$char]['prel'][$retk]) && $this->arabGlyphs[$char]['prel'][$retk]) {
                ...
                foreach ($this->arabGlyphs[$char]['prel'][$retk] as $k => $v) { // $k starts 0, 1...
                    if (!isset($chars[$i - $ig - $k])) {
                        $match = false;
                    } elseif (!str_contains($v, $chars[$i - $ig - $k])) {
                        while (str_contains($this->arabGlyphs[$char]['ignore'][$retk], $chars[$i - $ig - $k])) {  // ignore
                            ++$ig;
                        }
                        if (!isset($chars[$i - $ig - $k])) {
                            $match = false;
    ...
    ////////////////////////////////////////////////////////////////
    // Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
    public function TibetanlineBreaking(): void
    {
        for ($ptr = 0; $ptr < count($this->OTLdata); ++$ptr) {
            // Break opportunities at U+0F0B Tsheg or U=0F0D
            if (isset($this->OTLdata[$ptr]['uni']) && (0x0F0B == $this->OTLdata[$ptr]['uni'] || 0x0F0D == $this->OTLdata[$ptr]['uni'])) {
                if (isset($this->OTLdata[$ptr + 1]['uni']) && (0x0F0D == $this->OTLdata[$ptr + 1]['uni'] || 0xF0E == $this->OTLdata[$ptr + 1]['uni'])) {
                    continue;
                }
    ...
    // Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
    public function SEAlineBreaking(): void
    {
        ...
        while ($ptr < count($this->OTLdata) - 3) {
            ...
            if (1 == count($matches)) {
                ...
                if (isset($this->OTLdata[$matchpos + 1]['uni']) && (0x0E2F == $this->OTLdata[$matchpos + 1]['uni'] || 0x0E46 == $this->OTLdata[$matchpos + 1]['uni'] || 0x0EC6 == $this->OTLdata[$matchpos + 1]['uni'])) {
                    ++$matchpos;
                }
                // Set end of word marker in OTLdata at matchpos
    ...
    public function checkwordmatch(&$dict, $ptr)
    {
        ...
        while ($ok) {
            ...
            if (_DICT_INTERMEDIATE_MATCH == $x) {
                //echo "DICT_INTERMEDIATE_MATCH: ".dechex($c).'<br />';
                // Do not match if next character in text is a Mark
                if (isset($this->OTLdata[$ptr]['uni']) && !str_contains($this->GlyphClassMarks, $this->OTLdata[$ptr]['hex'])) {
                    $matches[] = $ptr - 1;
                }
                ++$dictptr;
            } elseif (_DICT_FINAL_MATCH == $x) {
                ...
                return $matches;
            } elseif (_DICT_NODE_TYPE_LINEAR == $x) {
                ...
                if ($c == $m) {
                    ...
                    if ($ptr > count($this->OTLdata) - 1) {
                        ...
                        if (_DICT_INTERMEDIATE_MATCH == $next || _DICT_FINAL_MATCH == $next) {
                            // Do not match if next character in text is a Mark
                            if (isset($this->OTLdata[$ptr]['uni']) && !str_contains($this->GlyphClassMarks, $this->OTLdata[$ptr]['hex'])) {
                                $matches[] = $ptr - 1;
                            }
                ...
                return $matches;
            } elseif (_DICT_NODE_TYPE_SPLIT == $x) {
    ...
    public function _applyGPOSrules($LookupList, $is_old_spec = false): void
    {
        foreach ($LookupList as $lu => $tag) {
    ...
    //////////////////////////////////////////////////////////////////////////////////
    public function _applyGPOSvaluerecord($basepos, $Value): void
    {
        ...
        if (str_contains($this->GlyphClassMarks, $this->OTLdata[$basepos]['hex'])) {
    ...
    public function _getXAdvancePos($pos)
    {
        ...
        if (str_contains($this->GlyphClassMarks, $this->OTLdata[$pos]['hex'])) {
            return $pos;
        }

        while (isset($this->OTLdata[$pos + 1]['hex']) && str_contains($this->GlyphClassMarks, $this->OTLdata[$pos + 1]['hex'])) {
            ++$pos;
        }
    ...
    public function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
    {
        if (($Flag & 0x0001) == 1) {
            $dir = 'RTL';
                ...
                for ($p = 0; $p < $PairSetCount; ++$p) {
                    if (isset($LuCoverage[$currGID]) && $LuCoverage[$currGID] == $p) {
                        ...
                        for ($pv = 0; $pv < $PairValueCount; ++$pv) {
                            ...
                            $checkpos = $ptr;
                            ++$checkpos;
                            while (isset($this->OTLdata[$checkpos]) && str_contains($ignore, $this->OTLdata[$checkpos]['hex'])) {
                                ++$checkpos;
                            }
        ...
        elseif (4 == $Type) {
            ...
            if ('I' == $this->shaper && $is_old_spec && !str_contains($this->GlyphClassMarks, $this->OTLdata[$ptr]['hex'])) {
                return;
            }
            ...
            if (isset($this->OTLdata[$checkpos]) && str_contains($BaseGlyphs, $this->OTLdata[$checkpos]['hex'])) {
                $matchedpos = $checkpos;
            } else {
                $matchedpos = false;
            }
            ...
            if (false !== $matchedpos) {
                ...
                [$x, $y] = $this->_getAnchorTable($BaseRecordOffset);
                ...
                $prevXPlacement = ($this->OTLdata[$matchedpos]['GPOSinfo']['XPlacement'] ?? 0);
                $prevYPlacement = ($this->OTLdata[$matchedpos]['GPOSinfo']['YPlacement'] ?? 0);
        ...
        elseif (5 == $Type) {
            ...
            if (isset($this->OTLdata[$checkpos]) && str_contains($LigatureGlyphs, $this->OTLdata[$checkpos]['hex'])) {
                $matchedpos = $checkpos;
            } else {
                $matchedpos = false;
            }
            ...
            if (false !== $matchedpos) {
                ...
                if (isset($this->assocLigs[$matchedpos], $this->assocMarks[$ptr]['ligPos']) && $this->assocMarks[$ptr]['ligPos'] == $matchedpos) {
                ...
                if (0 != $offset) {
                    ...
                    [$x, $y] = $this->_getAnchorTable($LigatureRecordOffset);
        ...
        elseif (6 == $Type) {
            ...
            if (isset($this->OTLdata[$checkpos]) && str_contains($Mark2Glyphs, $this->OTLdata[$checkpos]['hex'])) {
                $matchedpos = $checkpos;
            } else {
                $matchedpos = false;
            }
            ...
            if (false !== $matchedpos) {
                ...
                [$x, $y] = $this->_getAnchorTable($Mark2RecordOffset);
    ...
    public function checkContextMatch($Input, $Backtrack, $Lookahead, $ignore, $ptr)
    {
        ...
        $current_syllable = ($this->OTLdata[$ptr]['syllable'] ?? 0);
    ...
    public function checkContextMatchMultiple($Input, $Backtrack, $Lookahead, $ignore, $ptr, $class0excl = '', $bclass0excl = '', $lclass0excl = '')
    {
        ...
        for ($i = 0; $i < count($Backtrack); ++$i) {
            ...
            if (!$Backtrack[$i] && isset($this->OTLdata[$checkpos]) && str_contains($bclass0excl, $this->OTLdata[$checkpos]['hex'])) {
                return false;
            }
            if (!isset($this->OTLdata[$checkpos]) || !str_contains($Backtrack[$i], $this->OTLdata[$checkpos]['hex'])) {
                return false;
            }
        }
        ...
        for ($i = 1; $i < count($Input); ++$i) { // Start at 1 - already matched the first InputGlyph
            ...
            if (!$Input[$i] && isset($this->OTLdata[$checkpos]) && !str_contains($class0excl, $this->OTLdata[$checkpos]['hex'])) {
                $matched[] = $checkpos;
            } elseif (isset($this->OTLdata[$checkpos]) && str_contains($Input[$i], $this->OTLdata[$checkpos]['hex'])) {
                $matched[] = $checkpos;
            } else {
                return false;
            }
        ...
        for ($i = 0; $i < count($Lookahead); ++$i) {
            ...
            if (!$Lookahead[$i] && isset($this->OTLdata[$checkpos]) && str_contains($lclass0excl, $this->OTLdata[$checkpos]['hex'])) {
                return false;
            }
            if (!isset($this->OTLdata[$checkpos]) || !str_contains($Lookahead[$i], $this->OTLdata[$checkpos]['hex'])) {
                return false;
            }
        }
    ...
    public function checkContextMatchMultipleUni($Input, $Backtrack, $Lookahead, $ignore, $ptr, $class0excl = [], $bclass0excl = [], $lclass0excl = [])
    {
        ...
        for ($i = 0; $i < count($Backtrack); ++$i) {
            ...
            // If Class 0 specified, matches anything NOT in $bclass0excl
            if (!$Backtrack[$i] && isset($this->OTLdata[$checkpos], $bclass0excl[$this->OTLdata[$checkpos]['uni']])) {
                return false;
            }
        ...
        for ($i = 1; $i < count($Input); ++$i) { // Start at 1 - already matched the first InputGlyph
            ...
            if (!$Input[$i] && isset($this->OTLdata[$checkpos]) && !isset($class0excl[$this->OTLdata[$checkpos]['uni']])) {
                $matched[] = $checkpos;
            } elseif (isset($this->OTLdata[$checkpos], $Input[$i][$this->OTLdata[$checkpos]['uni']])) {
                $matched[] = $checkpos;
            } else {
                return false;
            }
        ...
        for ($i = 0; $i < count($Lookahead); ++$i) {
            ...
            // If Class 0 specified, matches anything NOT in $lclass0excl
            if (!$Lookahead[$i] && isset($this->OTLdata[$checkpos], $lclass0excl[$this->OTLdata[$checkpos]['uni']])) {
                return false;
            }
    ...
    public function _bidiSort($ta, $str, $dir, &$chunkOTLdata, $useGPOS)
    {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (8235 == $chunkOTLdata['char_data'][$i]['uni']) { // RLE
            ...
            } elseif (8234 == $chunkOTLdata['char_data'][$i]['uni']) { // LRE
            ...
            } elseif (8238 == $chunkOTLdata['char_data'][$i]['uni']) { // RLO
            ...
            } elseif (8237 == $chunkOTLdata['char_data'][$i]['uni']) { // LRO
            ...
            } elseif (8236 == $chunkOTLdata['char_data'][$i]['uni']) { // PDF
                // X7. With each PDF, determine the matching embedding or override code. If there was a valid matching code, restore (pop) the last remembered (pushed) embedding level and directional override.
                if (count($remember)) {
                    $last = count($remember) - 1;
                    if ((8235 == $remember[$last]['num']) || (8234 == $remember[$last]['num']) || (8238 == $remember[$last]['num'])
                        || (8237 == $remember[$last]['num'])) {
                        $match = array_pop($remember);
                        $cel = $match['cel'];
                        $dos = $match['dos'];
                    }
                }
            } elseif (10 == $chunkOTLdata['char_data'][$i]['uni']) { // NEW LINE
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (UCDN::BIDI_CLASS_NSM == $chardata[$i]['type']) {
                if (0 == $i || $chardata[$i]['level'] != $chardata[$i - 1]['level']) {
        ...
        $levcount = 0;
        for ($i = 0; $i < $numchars; ++$i) {
            if (UCDN::BIDI_CLASS_EN == $chardata[$i]['type']) {
                $found = false;
                for ($j = $levcount; $j >= 0; --$j) {
                    if (UCDN::BIDI_CLASS_AL == $chardata[$j]['type']) {
                        $chardata[$i]['type'] = UCDN::BIDI_CLASS_AN;
                        $found = true;

                        break;
                    }
                    if ((UCDN::BIDI_CLASS_L == $chardata[$j]['type']) || (UCDN::BIDI_CLASS_R == $chardata[$j]['type'])) {
                        $found = true;
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (UCDN::BIDI_CLASS_AL == $chardata[$i]['type']) {
        ...
        for ($i = 1; $i < $numchars; ++$i) {
            if (($i + 1) < $numchars && $chardata[($i)]['level'] == $chardata[($i + 1)]['level'] && $chardata[($i)]['level'] == $chardata[($i - 1)]['level']) {
                if (UCDN::BIDI_CLASS_ES == $chardata[$i]['type'] && UCDN::BIDI_CLASS_EN == $chardata[($i - 1)]['type'] && UCDN::BIDI_CLASS_EN == $chardata[($i + 1)]['type']) {
                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;
                } elseif (UCDN::BIDI_CLASS_CS == $chardata[$i]['type'] && UCDN::BIDI_CLASS_EN == $chardata[($i - 1)]['type'] && UCDN::BIDI_CLASS_EN == $chardata[($i + 1)]['type']) {
                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;
                } elseif (UCDN::BIDI_CLASS_CS == $chardata[$i]['type'] && UCDN::BIDI_CLASS_AN == $chardata[($i - 1)]['type'] && UCDN::BIDI_CLASS_AN == $chardata[($i + 1)]['type']) {
                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_AN;
                }
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (UCDN::BIDI_CLASS_ET == $chardata[$i]['type']) {
                if ($i > 0 && UCDN::BIDI_CLASS_EN == $chardata[($i - 1)]['type'] && $chardata[($i)]['level'] == $chardata[($i - 1)]['level']) {
                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;
                } else {
                    $j = $i + 1;
                    while ($j < $numchars && $chardata[$j]['level'] == $chardata[$i]['level']) {
                        if (UCDN::BIDI_CLASS_EN == $chardata[$j]['type']) {
                            $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;

                            break;
                        }
                        if (UCDN::BIDI_CLASS_ET != $chardata[$j]['type']) {
                            break;
                        }
                        ++$j;
                    }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ((UCDN::BIDI_CLASS_ET == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_ES == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_CS == $chardata[$i]['type'])) {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (UCDN::BIDI_CLASS_EN == $chardata[$i]['type']) {
                if (0 == $i) { // Start of Level run
                    if (UCDN::BIDI_CLASS_L == $chardata[$i]['sor']) {
                        $chardata[$i]['type'] = $chardata[$i]['sor'];
                    }
                } else {
                    for ($j = $i - 1; $j >= 0; --$j) {
                        if ($chardata[$j]['level'] != $chardata[$i]['level']) { // Level run boundary
                            if (UCDN::BIDI_CLASS_L == $chardata[$j + 1]['sor']) {
                                $chardata[$i]['type'] = $chardata[$j + 1]['sor'];
                            }

                            break;
                        }
                        if (UCDN::BIDI_CLASS_L == $chardata[$j]['type']) {
                            $chardata[$i]['type'] = UCDN::BIDI_CLASS_L;

                            break;
                        }
                        if (UCDN::BIDI_CLASS_R == $chardata[$j]['type']) {
                            break;
                        }
                    }
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if (UCDN::BIDI_CLASS_ON == $chardata[$i]['type'] || UCDN::BIDI_CLASS_WS == $chardata[$i]['type']) {
                $left = -1;
                // LEFT
                if (0 == $i) {  // first char
                    $left = $chardata[($i)]['sor'];
                    ...
                    $left = $chardata[($i)]['sor'];
                } elseif (UCDN::BIDI_CLASS_L == $chardata[($i - 1)]['type']) {
                    $left = UCDN::BIDI_CLASS_L;
                } elseif (UCDN::BIDI_CLASS_R == $chardata[($i - 1)]['type'] || UCDN::BIDI_CLASS_EN == $chardata[($i - 1)]['type'] || UCDN::BIDI_CLASS_AN == $chardata[($i - 1)]['type']) {
                ...
                while ((UCDN::BIDI_CLASS_ON == $chardata[$j]['type'] || UCDN::BIDI_CLASS_WS == $chardata[$j]['type']) && $j <= ($numchars - 1)) {
                    ...
                    if (UCDN::BIDI_CLASS_L == $chardata[($j + 1)]['type']) {
                        $right = UCDN::BIDI_CLASS_L;

                        break;
                    }
                    if (UCDN::BIDI_CLASS_R == $chardata[($j + 1)]['type'] || UCDN::BIDI_CLASS_EN == $chardata[($j + 1)]['type'] || UCDN::BIDI_CLASS_AN == $chardata[($j + 1)]['type']) {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            ...
            if ($odd) {
                if ((UCDN::BIDI_CLASS_L == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_AN == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_EN == $chardata[$i]['type'])) {
                    ++$chardata[$i]['level'];
                }
            } else {
                if (UCDN::BIDI_CLASS_R == $chardata[$i]['type']) {
                    ++$chardata[$i]['level'];
                } elseif ((UCDN::BIDI_CLASS_AN == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_EN == $chardata[$i]['type'])) {
                    $chardata[$i]['level'] += 2;
                }
            }
...
//
        ...
        for ($i = ($numchars - 1); $i > 0; --$i) {
            if (UCDN::BIDI_CLASS_WS == $chardata[$i]['type'] || (isset($chardata[$i]['orig_type']) && UCDN::BIDI_CLASS_WS == $chardata[$i]['orig_type'])) {
        ...
        for ($j = $maxlevel; $j > 0; --$j) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($chardata[$i]['level'] >= $j) {
                    ...
                    if (isset(UCDN::$mirror_pairs[$chardata[$i]['char']]) && UCDN::BIDI_CLASS_R == $chardata[$i]['type']) {
        ...
        foreach ($chardata as $cd) {
            ...
            if (UCDN::BIDI_CLASS_L == $cd['type']) {
                $rtl_content |= 1;
            } elseif (UCDN::BIDI_CLASS_R == $cd['type']) {
                $rtl_content |= 2;
            }
            ++$cctr;
        }
    ...
    public function _bidiPrepare(&$para, $dir): void
    {
        // Set the initial paragraph embedding level
        $pel = 0; // paragraph embedding level
        if ('rtl' == $dir) {
        ...
        for ($nc = 0; $nc < $numchunks; ++$nc) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if (8235 == $chunkOTLdata['char_data'][$i]['uni']) { // RLE
                ...
                } elseif (8294 == $chunkOTLdata['char_data'][$i]['uni'] || 8295 == $chunkOTLdata['char_data'][$i]['uni']
                    || 8296 == $chunkOTLdata['char_data'][$i]['uni']) { // LRI // RLI // FSI
                    // X5a. With each RLI:
                    // X5b. With each LRI:
                    ...
                    if (8296 == $chunkOTLdata['char_data'][$i]['uni']) { // FSI
                        $lvl = 0;
                        $nc2 = $nc;
                        $i2 = $i;
                        while (!($nc2 == ($numchunks - 1) && $i2 == ((count($para[$nc2][18]['char_data'])) - 1))) {  // while not at end of last chunk
                            ...
                            if (8294 == $para[$nc2][18]['char_data'][$i2]['uni'] || 8295 == $para[$nc2][18]['char_data'][$i2]['uni'] || 8296 == $para[$nc2][18]['char_data'][$i2]['uni']) {
                                ++$lvl;

                                continue;
                            }
                            if (8297 == $para[$nc2][18]['char_data'][$i2]['uni']) {
                                --$lvl;
                                if ($lvl < 0) {
                                    break;
                                }
                            }
                            if (UCDN::BIDI_CLASS_L === $para[$nc2][18]['char_data'][$i2]['bidi_class'] || UCDN::BIDI_CLASS_AL == $para[$nc2][18]['char_data'][$i2]['bidi_class'] || UCDN::BIDI_CLASS_R === $para[$nc2][18]['char_data'][$i2]['bidi_class']) {
                    ...
                    if (8294 == $chunkOTLdata['char_data'][$i]['uni'] || UCDN::BIDI_CLASS_L === $fsi) { // LRI or FSI-L
                        ...
                        $next_level = $cel + 2 - ($cel % 2);
                    } elseif (8295 == $chunkOTLdata['char_data'][$i]['uni'] || UCDN::BIDI_CLASS_R == $fsi || UCDN::BIDI_CLASS_AL == $fsi) { // RLI or FSI-R
                    ...
                    $controlchars = true;
                } elseif (8297 == $chunkOTLdata['char_data'][$i]['uni']) { // PDI
                    ...
                    while (count($remember)) {
                        $last = count($remember) - 1;
                        if ((8294 == $remember[$last]['num']) || (8295 == $remember[$last]['num']) || (8296 == $remember[$last]['num'])) {
                            $match = array_pop($remember);
                            $cel = $match['cel'];
                            $dos = $match['dos'];
                            $diid = $match['diid'];
                    ...
                    if (-1 != $dos) {
                        ...
                        $chardir = $chunkOTLdata['char_data'][$i]['bidi_class'];
                        if (UCDN::BIDI_CLASS_R == $chardir || UCDN::BIDI_CLASS_AL == $chardir) {
                            $strongrtl = true;
                        }
                    }
                    $chunkOTLdata['char_data'][$i]['level'] = $cel;
        ...
        for ($nc = ($numchunks - 1); $nc >= 0; --$nc) {
            if (0 == count($para[$nc][18]['char_data'])) {
                array_splice($para, $nc, 1);
            }
        }
        if ('rtl' != $dir && !$strongrtl && !$controlchars) {
            return;
        }
        ...
        for ($ir = 0; $ir <= $dictr; ++$ir) {
            ...
            for ($nc = 0; $nc < $numchunks; ++$nc) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (UCDN::BIDI_CLASS_EN == $chardata[$i]['type'] && UCDN::BIDI_CLASS_AL == $laststrongtype) {
                        $chardata[$i]['type'] = UCDN::BIDI_CLASS_AN;
                    }
                    if (UCDN::BIDI_CLASS_L == $chardata[$i]['type'] || UCDN::BIDI_CLASS_R == $chardata[$i]['type'] || UCDN::BIDI_CLASS_AL == $chardata[$i]['type']) {
        ...
        for ($nc = 0; $nc < $numchunks; ++$nc) {
            ...
            $numchars = count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && UCDN::BIDI_CLASS_AL == $chardata[$i]['type']) {
                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_R;
                }
            }
        }
            ...
            for ($nc = 0; $nc < $numchunks; ++$nc) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (!isset($chardata[$i]['sor']) && !isset($chardata[$i]['eor'])) {
                        if (UCDN::BIDI_CLASS_ES == $chardata[$i]['type'] && UCDN::BIDI_CLASS_EN == $prevtype && UCDN::BIDI_CLASS_EN == $nexttype) {
                            $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;
                        } elseif (UCDN::BIDI_CLASS_CS == $chardata[$i]['type'] && UCDN::BIDI_CLASS_EN == $prevtype && UCDN::BIDI_CLASS_EN == $nexttype) {
                            $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;
                        } elseif (UCDN::BIDI_CLASS_CS == $chardata[$i]['type'] && UCDN::BIDI_CLASS_AN == $prevtype && UCDN::BIDI_CLASS_AN == $nexttype) {
                            $chardata[$i]['type'] = UCDN::BIDI_CLASS_AN;
                        }
                    }
                    $prevtype = $chardata[$i]['type'];
        ...
        for ($ir = 0; $ir <= $dictr; ++$ir) {
            ...
            for ($nc = 0; $nc < $numchunks; ++$nc) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (UCDN::BIDI_CLASS_ET == $chardata[$i]['type']) {
                        if (UCDN::BIDI_CLASS_EN == $prevtype) {
                            $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;
                            ...
                            while (!($nc2 == ($numchunks - 1) && $i2 == ((count($para[$nc2][18]['char_data'])) - 1))) { // while not at end of last chunk
                                ...
                                if (UCDN::BIDI_CLASS_EN == $nexttype) {
                                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_EN;

                                    break;
                                }
                                if (UCDN::BIDI_CLASS_ET != $nexttype) {
                                    break;
                                }
                            }
                        }
        ...
        for ($nc = 0; $nc < $numchunks; ++$nc) {
            ...
            $numchars = count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && ((UCDN::BIDI_CLASS_ET == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_ES == $chardata[$i]['type']) || (UCDN::BIDI_CLASS_CS == $chardata[$i]['type']))) {
                    $chardata[$i]['type'] = UCDN::BIDI_CLASS_ON;
                }
            }
        }
        ...
        for ($ir = 0; $ir <= $dictr; ++$ir) {
            ...
            for ($nc = 0; $nc < $numchunks; ++$nc) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (isset($chardata[$i]['type']) && UCDN::BIDI_CLASS_EN == $chardata[$i]['type'] && UCDN::BIDI_CLASS_L == $laststrongtype) {
                        $chardata[$i]['type'] = UCDN::BIDI_CLASS_L;
                    }
                    if (isset($chardata[$i]['type']) && (UCDN::BIDI_CLASS_L == $chardata[$i]['type'] || UCDN::BIDI_CLASS_R == $chardata[$i]['type'] || UCDN::BIDI_CLASS_AL == $chardata[$i]['type'])) {
                        $laststrongtype = $chardata[$i]['type'];
                    }
                }
            }
            ...
            for ($nc = 0; $nc < $numchunks; ++$nc) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (UCDN::BIDI_CLASS_ON == $chardata[$i]['type'] || UCDN::BIDI_CLASS_WS == $chardata[$i]['type']) {
                        $left = -1;
                        // LEFT
                        if (UCDN::BIDI_CLASS_R == $laststrongtype || UCDN::BIDI_CLASS_EN == $laststrongtype || UCDN::BIDI_CLASS_AN == $laststrongtype) {
                            $left = UCDN::BIDI_CLASS_R;
                        } elseif (UCDN::BIDI_CLASS_L == $laststrongtype) {
                            $left = UCDN::BIDI_CLASS_L;
                        }
                        // RIGHT
                        $right = -1;
                        ...
                        if (isset($chardata[$i]['eor'])) {
                            ...
                            while (!($nc2 == ($numchunks - 1) && $i2 == ((count($para[$nc2][18]['char_data'])) - 1))) { // while not at end of last chunk
                                ...
                                $nexttype = $para[$nc2][18]['char_data'][$i2]['type'];
                                if (UCDN::BIDI_CLASS_R == $nexttype || UCDN::BIDI_CLASS_EN == $nexttype || UCDN::BIDI_CLASS_AN == $nexttype) {
                                    $right = UCDN::BIDI_CLASS_R;

                                    break;
                                }
                                if (UCDN::BIDI_CLASS_L == $nexttype) {
                                    $right = UCDN::BIDI_CLASS_L;
                    ...
                    } elseif (UCDN::BIDI_CLASS_L == $chardata[$i]['type'] || UCDN::BIDI_CLASS_R == $chardata[$i]['type'] || UCDN::BIDI_CLASS_EN == $chardata[$i]['type'] || UCDN::BIDI_CLASS_AN == $chardata[$i]['type']) {
                        $laststrongtype = $chardata[$i]['type'];
                    }
                }
            }
        ...
        for ($nc = 0; $nc < $numchunks; ++$nc) {
            ...
            $numchars = count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && (UCDN::BIDI_CLASS_ON == $chardata[$i]['type'] || UCDN::BIDI_CLASS_WS == $chardata[$i]['type'])) {
                    $chardata[$i]['orig_type'] = $chardata[$i]['type']; // Need to store the original 'WS' for reference in L1 below
                    $chardata[$i]['type'] = ($chardata[$i]['level'] % 2) ? UCDN::BIDI_CLASS_R : UCDN::BIDI_CLASS_L;
                }
            }
    ...
    public function _bidiReorder(&$chunkorder, &$content, &$cOTLdata, $blockdir): void
    {
        $bidiData = [];
        ...
        for ($nc = 0; $nc < $numchunks; ++$nc) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                ...
                $maxlevel = max(($carac['level'] ?? 0), $maxlevel);
                $bidiData[] = $carac;
            }
        }
        if (0 == $maxlevel) {
        ...
        for ($i = ($numchars - 1); $i > 0; --$i) {
            if (UCDN::BIDI_CLASS_WS == $bidiData[$i]['type'] || (isset($bidiData[$i]['orig_type']) && UCDN::BIDI_CLASS_WS == $bidiData[$i]['orig_type'])) {
        ...
        for ($j = $maxlevel; $j > 0; --$j) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($bidiData[$i]['level'] >= $j) {
                    $onlevel = true;
                    // L4. A character is depicted by a mirrored glyph if and only if (a) the resolved directionality of that character is R, and (b) the Bidi_Mirrored property value of that character is true.
                    if (isset(UCDN::$mirror_pairs[$bidiData[$i]['uni']]) && UCDN::BIDI_CLASS_R == $bidiData[$i]['type']) {
        ...
        foreach ($bidiData as $carac) {
            ...
            if (0xFFFC != $carac['uni']) {   // Object replacement character (65532)
    ...
    public function removeChar(&$txt, &$cOTLdata, $char): void
    {
        while (false !== mb_strpos($txt, $char, 0, $this->mpdf->mb_enc)) {
    ...
    // Remove one or more occurrences of $char (single character) from $txt and adjust OTLdata
    public function replaceSpace(&$txt, &$cOTLdata): void
    {
        $char = chr(194).chr(160); // NBSP
        while (false !== mb_strpos($txt, $char, 0, $this->mpdf->mb_enc)) {
            $pos = mb_strpos($txt, $char, 0, $this->mpdf->mb_enc);
            if (160 == $cOTLdata['char_data'][$pos]['uni']) {
    ...
    public function trimOTLdata(&$cOTLdata, $Left = true, $Right = true): void
    {
        $len = count($cOTLdata['char_data']);
        $nLeft = 0;
        $nRight = 0;
        for ($i = 0; $i < $len; ++$i) {
            if (32 == $cOTLdata['char_data'][$i]['uni'] || 12288 == $cOTLdata['char_data'][$i]['uni']) {
                ++$nLeft;
            } // 12288 = 0x3000 = CJK space
            else {
                break;
    ...
    public function _getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $shaper, $useOTL, $mode)
    {
        ...
        } elseif (UCDN::SCRIPT_CYRILLIC == $scriptblock) {
            if (!($useOTL & 0x02)) {
                return ['', false];
            }
        } elseif (UCDN::SCRIPT_GREEK == $scriptblock) {
            if (!($useOTL & 0x04)) {
                return ['', false];
            }
    ...
    public function _getOTLLangTag($ietf, $available)
    {
        ...
        if (!str_contains($available, $langsys)) {
            if (str_contains($available, 'DFLT')) {
                return 'DFLT';
            }
    ...
    public function _dumpproc($GPOSSUB, $lookupID, $subtable, $Type, $Format, $ptr, $currGlyph, $level): void
    {
