...
class grad
{
    ...
    public function CoonsPatchMesh($x, $y, $w, $h, $patch_array = [], $x_min = 0, $x_max = 1, $y_min = 0, $y_max = 1, $colspace = 'RGB', $return = false)
    {
        ...
        for ($i = 0; $i < count($patch_array); ++$i) {
            ...
            for ($j = 0; $j < count($patch_array[$i]['colors']); ++$j) {
                ...
                } elseif ('Gray' == $colspace) {
                    $this->mpdf->gradients[$n]['stream'] .= ($patch_array[$i]['colors'][$j][1]);
                    if (1 == $patch_array[$i]['colors'][$j][2]) {
                        $trans = true;
    ...
    {
        if ('L' == strtoupper(substr($type, 0, 1))) {
            $type = 2;
        } // linear
        elseif ('R' == strtoupper(substr($type, 0, 1))) {
            $type = 3;
        } // radial
        if ('CMYK' != $colorspace && 'Gray' != $colorspace) {
            $colorspace = 'RGB';
        }
        $bboxw = $w;
        $bboxh = $h;
        ...
        if (false !== $coords[0] && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $coords[0], $m)) {
            $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
            if ($tmp) {
                $coords[0] = $tmp / $w;
            }
        }
        if (false !== $coords[1] && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $coords[1], $m)) {
            $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
            if ($tmp) {
                $coords[1] = 1 - ($tmp / $h);
            }
        ...
        // LINEAR
        if (2 == $type) {
            $angle = ($coords[4] ?? false);
            $repeat = ($coords[5] ?? false);
            // ALL POINTS SET (default for custom mPDF linear gradient) - no -moz
            if (false !== $coords[0] && false !== $coords[1] && false !== $coords[2] && false !== $coords[3]) {
            ...
            elseif (false !== $angle && false !== $coords[0] && false !== $coords[1] && false === $coords[2] && false === $coords[3]) {
            ...
            elseif (false !== $angle && false === $coords[0] && false === $coords[1]) {
                if (0 == $angle || 360 == $angle) {
                    $coords[0] = 0;
                    $coords[1] = 0;
                    $coords[2] = 1;
            ...
            elseif ((!isset($angle) || false === $angle) && false !== $coords[0] && false !== $coords[1]) {  // should have start and end defined
                $coords[2] = 1 - $coords[0];
                $coords[3] = 1 - $coords[1];
        ...
        elseif (3 == $type) {
            ...
            $angle = ($coords[5] ?? false); // ?? no effect
            $shape = ($coords[6] ?? false);
            $size = ($coords[7] ?? false);
            $repeat = ($coords[8] ?? false);
            // ALL POINTS AND RADIUS SET (default for custom mPDF radial gradient) - no -moz
            if (false !== $coords[0] && false !== $coords[1] && false !== $coords[2] && false !== $coords[3] && false !== $coords[4]) {
                // do nothing - coords used as they are
            }
            // If a <point> is defined
            elseif (false !== $shape && false !== $size) {
                if (false == $coords[2]) {
                    $coords[2] = $coords[0];
                }
                if (false == $coords[3]) {
        ...
        for ($i = 0; $i < count($stops); ++$i) {
            ...
            if ($i > 0 && $i < (count($stops) - 1)) {
                if (!isset($stops[$i]['offset']) || (isset($stops[$i + 1]['offset']) && $stops[$i]['offset'] > $stops[$i + 1]['offset']) || $stops[$i]['offset'] < $stops[$i - 1]['offset']) {
                    if (isset($stops[$i - 1]['offset'], $stops[$i + 1]['offset'])) {
    ...
    public function parseMozGradient($bg)
    {
        ...
        if (preg_match('/linear-gradient\((.*)\)/', $bg, $m)) {
            ...
            if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i', $bgr[0]) && !preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0])) {
                $startStops = 1;
            } elseif ('0' === trim($first[(count($first) - 1)])) {
                $startStops = 1;
            } else {
            ...
            if (1 == $startStops) { // default values
    ...
    public function parseBackgroundGradient($bg)
    {
        ...
        $g = [];
        if (count($bgr) > 6) {
            if ('L' == strtoupper(substr($bgr[0], 0, 1)) && 7 == count($bgr)) {  // linear
                $g['type'] = 2;
