...
class SVG
{
    ...
    public function svgImage($attribs): void
    {
        // x and y are coordinates
        $x = ($attribs['x'] ?? 0);
        $y = ($attribs['y'] ?? 0);
        // preserveAspectRatio
        $par = ($attribs['preserveAspectRatio'] ?? 'xMidYMid meet');
        // width and height are <lengths> - Required attributes
        $wset = ($attribs['width'] ?? 0);
        $hset = ($attribs['height'] ?? 0);
        ...
        $orig_srcpath = '';
        if ('' != trim($srcpath) && 'var:' == substr($srcpath, 0, 4)) {
        ...
        if ('none' != $ar[0]) { // If "none" need to do nothing
            //  Force uniform scaling
            if (isset($ar[1]) && 'slice' == $ar[1]) {
                $meetOrSlice = 'slice';
            } else {
                $meetOrSlice = 'meet';
            }
            if ($info['h'] / $info['w'] > $h / $w) {
                if ('meet' == $meetOrSlice) { // the entire viewBox is visible within the viewport
            ...
            if ('xminymin' == $ar[0]) {
                // do nothing to x
                // do nothing to y
            } elseif ('xmidymin' == $ar[0]) {
                $img_x += $w / 2 - $img_w / 2; // xMid
                // do nothing to y
            } elseif ('xmaxymin' == $ar[0]) {
                $img_x += $w - $img_w; // xMax
                // do nothing to y
            } elseif ('xminymid' == $ar[0]) {
                // do nothing to x
                $img_y += $h / 2 - $img_h / 2; // yMid
            } elseif ('xmaxymid' == $ar[0]) {
                $img_x += $w - $img_w; // xMax
                $img_y += $h / 2 - $img_h / 2; // yMid
            } elseif ('xminymax' == $ar[0]) {
                // do nothing to x
                $img_y += $h - $img_h; // yMax
            } elseif ('xmidymax' == $ar[0]) {
                $img_x += $w / 2 - $img_w / 2; // xMid
                $img_y += $h - $img_h; // yMax
            } elseif ('xmaxymax' == $ar[0]) {
                $img_x += $w - $img_w; // xMax
                $img_y += $h - $img_h; // yMax
        ...
        // Output
        if ('slice' == $meetOrSlice) { // need to add a clipping path to reference rectangle
        ...
        if ('slice' == $meetOrSlice) { // need to end clipping path
    ...
    public function svgGradient($gradient_info, $attribs, $element)
    {
        ...
        if ('rect' == $element) {
            ...
            if (is_array($this->pathBBox) && $this->pathBBox[2] > 0) {
                ...
                foreach ($commands as $c) {
                    if (3 == count($c)) {
                        [$tmp, $cmd, $arg] = $c;
                        if ('M' == $cmd || 'L' == $cmd || 'C' == $cmd || 'S' == $cmd || 'Q' == $cmd || 'T' == $cmd) {
        ...
        if (isset($gradient_info['units']) && 'userspaceonuse' == strtolower($gradient_info['units'])) {
            if ($transformations) {
        ...
        if (isset($gradient_info['spread'])) {
            if ('reflect' == strtolower($gradient_info['spread'])) {
                $spread = 'F';
            } // reflect
            elseif ('repeat' == strtolower($gradient_info['spread'])) {
                $spread = 'R';
            } // repeat
        }

        for ($i = 0; $i < (count($gradient_info['color'])); ++$i) {
            if (false !== stristr($gradient_info['color'][$i]['offset'], '%')) {
                $gradient_info['color'][$i]['offset'] = ($gradient_info['color'][$i]['offset'] + 0) / 100;
            }
            if (isset($gradient_info['color'][($i + 1)]['offset']) && false !== stristr($gradient_info['color'][($i + 1)]['offset'], '%')) {
        ...
        if ('linear' == $gradient_info['type']) {
            ...
            if (isset($gradient_info['units']) && 'userspaceonuse' == strtolower($gradient_info['units'])) {
                ...
                if (0 != $angle && 360 != $angle && 90 != $angle && 180 != $angle && 270 != $angle) {
                    if ($w >= $h) {
                        $y1 *= $h / $w;
                        $y2 *= $h / $w;
                        $usew = $useh = $bboxw;
            ...
            if (isset($gradient_info['units']) && 'objectboundingbox' == strtolower($gradient_info['units'])) {
                if ($transformations) {
                    $return .= $transformations;
                }
            }
            ...
            if ('R' == $spread || 'F' == $spread) { // Repeat  /  Reflect
                $offs = [];
                for ($i = 0; $i < $ns; ++$i) {
            ...
            for ($i = 0; $i < ($stops); ++$i) {
                if (!$gradient_info['color'][$i]['color']) {
                    if ('RGB' == $gradient_info['colorspace']) {
                        $gradient_info['color'][$i]['color'] = '0 0 0';
                    } elseif ('Gray' == $gradient_info['colorspace']) {
                        $gradient_info['color'][$i]['color'] = '0';
                    } elseif ('CMYK' == $gradient_info['colorspace']) {
            ...
            $return .= ' /Sh'.($n).' sh ';
            $return .= " Q\n";
        } elseif ('radial' == $gradient_info['type']) {
            if (isset($gradient_info['units']) && 'userspaceonuse' == strtolower($gradient_info['units'])) {
                if ($w > $h) {
                    $h = $w;
                } else {
    ...
    public function svgOffset($attribs): void
    {
    ...
    public function svg_overflow($x, $y)
    {
        ...
        if (isset($this->svg_attribs['overflow'])) {
            if ('hidden' == $this->svg_attribs['overflow']) {
    ...
    public function svgDefineStyle($critere_style)
    {
        ...
        if (isset($critere_style['style'])) {
            if (preg_match('/fill:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)/i', $critere_style['style'], $m)) { // mPDF 5.7.2
                ...
                if ($tmp && 'inherit' != $tmp && $tmp != $critere_style['style']) {
            ...
            if ((preg_match('/[^-]opacity:\\s*([a-z0-9.]*|none)/i', $critere_style['style'], $m)
                || preg_match('/^opacity:\\s*([a-z0-9.]*|none)/i', $critere_style['style'], $m)) && 'inherit' != $m[1]) {
        ...
        // mPDF 5.7.2
        if (isset($critere_style['opacity']) && 'inherit' != $critere_style['opacity']) {
            $current_style['fill-opacity'] = $critere_style['opacity'];
            $current_style['stroke-opacity'] = $critere_style['opacity'];
        }

        if (isset($critere_style['fill']) && 'inherit' != $critere_style['fill']) {
            $current_style['fill'] = $critere_style['fill'];
        }

        if (isset($critere_style['fill-opacity']) && 'inherit' != $critere_style['fill-opacity']) {
            $current_style['fill-opacity'] = $critere_style['fill-opacity'];
        }

        if (isset($critere_style['fill-rule']) && 'inherit' != $critere_style['fill-rule']) {
            $current_style['fill-rule'] = $critere_style['fill-rule'];
        }

        if (isset($critere_style['stroke']) && 'inherit' != $critere_style['stroke']) {
            $current_style['stroke'] = $critere_style['stroke'];
        }

        if (isset($critere_style['stroke-linecap']) && 'inherit' != $critere_style['stroke-linecap']) {
            $current_style['stroke-linecap'] = $critere_style['stroke-linecap'];
        }

        if (isset($critere_style['stroke-linejoin']) && 'inherit' != $critere_style['stroke-linejoin']) {
            $current_style['stroke-linejoin'] = $critere_style['stroke-linejoin'];
        }

        if (isset($critere_style['stroke-miterlimit']) && 'inherit' != $critere_style['stroke-miterlimit']) {
            $current_style['stroke-miterlimit'] = $critere_style['stroke-miterlimit'];
        }

        if (isset($critere_style['stroke-opacity']) && 'inherit' != $critere_style['stroke-opacity']) {
            $current_style['stroke-opacity'] = $critere_style['stroke-opacity'];
        }

        if (isset($critere_style['stroke-width']) && 'inherit' != $critere_style['stroke-width']) {
            $current_style['stroke-width'] = $critere_style['stroke-width'];
        }

        if (isset($critere_style['stroke-dasharray']) && 'inherit' != $critere_style['stroke-dasharray']) {
            $current_style['stroke-dasharray'] = $critere_style['stroke-dasharray'];
        }
        if (isset($critere_style['stroke-dashoffset']) && 'inherit' != $critere_style['stroke-dashoffset']) {
            $current_style['stroke-dashoffset'] = $critere_style['stroke-dashoffset'];
        }

        // Used as indirect setting for currentColor
        if (isset($critere_style['color']) && 'inherit' != $critere_style['color']) {
    ...
    public function svgStyle($critere_style, $attribs, $element)
    {
        ...
        $w = '';
        $style = '';
        if (substr_count($critere_style['fill'], 'url') > 0 && 'line' != $element) {
            //
            // couleur degradé
        ...
        // Used as indirect setting for currentColor
        elseif ('currentcolor' == strtolower($critere_style['fill']) && 'line' != $element) {
        ...
        } elseif ('none' != $critere_style['fill'] && 'line' != $element) {
        ...
        // Used as indirect setting for currentColor
        elseif ('currentcolor' == strtolower($critere_style['stroke'])) {
            $col = $this->mpdf_ref->ConvertColor($critere_style['color']);
            if ($col) {
                if (5 == $col[0]) {
        ...
        } elseif ('none' != $critere_style['stroke']) {
        ...
        if ('none' != $critere_style['stroke']) {
            if ('miter' == $critere_style['stroke-linejoin']) {
                $path_style .= ' 0 j ';
            } elseif ('round' == $critere_style['stroke-linejoin']) {
                $path_style .= ' 1 j ';
            } elseif ('bevel' == $critere_style['stroke-linejoin']) {
                $path_style .= ' 2 j ';
            }

            if ('butt' == $critere_style['stroke-linecap']) {
                $path_style .= ' 0 J ';
            } elseif ('round' == $critere_style['stroke-linecap']) {
                $path_style .= ' 1 J ';
            } elseif ('square' == $critere_style['stroke-linecap']) {
                $path_style .= ' 2 J ';
            }

            if (isset($critere_style['stroke-miterlimit'])) {
                if ('none' == $critere_style['stroke-miterlimit']) {
        ...
        if ('evenodd' == $critere_style['fill-rule']) {
            $fr = '*';
        } else {
            $fr = '';
        }
        ...
        if (isset($critere_style['fill-opacity'])) {
            $opacity = 1;
            if (0 == $critere_style['fill-opacity']) {
                $opacity = 0;
        ...
        if (isset($critere_style['stroke-opacity'])) {
            $opacity = 1;
            if (0 == $critere_style['stroke-opacity']) {
                $opacity = 0;
    ...
    public function svgPath($command, $arguments)
    {
        ...
        switch (strtolower($command)) {
            ...
            case 's': // bezier with first vertex equal first control
                // mPDF 4.4.003
                if (!('C' == $this->lastcommand || 'c' == $this->lastcommand || 'S' == $this->lastcommand || 's' == $this->lastcommand)) {
            ...
            case 't': // bezier quadratic avec point de control simetrique a lancien point de control
                if (!('Q' == $this->lastcommand || 'q' == $this->lastcommand || 'T' == $this->lastcommand || 't' == $this->lastcommand)) {
                    $this->lastcontrolpoints = [0, 0];
                }
                for ($i = 0; $i < $ile_argumentow; $i += 2) {
                    $x = ($a[$i][0]);
            ...
            case 'a': // Elliptical arc
                for ($i = 0; $i < $ile_argumentow; $i += 7) {
                    ...
                    [$pcmd, $bounds] = $this->Arcto($x1, $y1, $x2, $y2, $rx, $ry, $angle, $largeArcFlag, $sweepFlag);
    ...
    public function ConvertSVGSizePixels($size = 5, $maxsize = 'x')
    {
        ...
        //Return as pixels
        return $this->mpdf_ref->ConvertSize($size, $maxsize, $fontsize, false) * 1 / (25.4 / $this->mpdf_ref->dpi);
    }
    ...
    public function ConvertSVGSizePts($size = 5)
    {
        ...
        //Return as pts
        return $this->mpdf_ref->ConvertSize($size, $maxsize, false, true) * 72 / 25.4;
    }
    ...
    {
        if (0 == $arguments['h'] || 0 == $arguments['w']) {
            return '';
        }
    ...
    {
        if (0 == $arguments['rx'] || 0 == $arguments['ry']) {
            return '';
        }
    ...
    public function svgText()
    {
        ...
        if (isset($this->txt_data[2])) {
            ...
            if (isset($this->svg_font[$current_style['font-family']])) {
                // select font
                $style = 'R';
                $style .= (isset($current_style['font-weight']) && 'bold' == $current_style['font-weight']) ? 'B' : '';
                $style .= (isset($current_style['font-style']) && 'italic' == $current_style['font-style']) ? 'I' : '';
                $style .= (isset($current_style['font-variant']) && 'small-caps' == $current_style['font-variant']) ? 'S' : '';
                ...
                if (isset($current_style['fill-opacity'])) {
                    if (0 == $current_style['fill-opacity']) {
                        $fopacity = 0;
                ...
                if (isset($current_style['stroke-opacity'])) {
                    if (0 == $current_style['stroke-opacity']) {
                        $sopacity = 0;
                ...
                $gs = $this->mpdf_ref->AddExtGState(['ca' => $fopacity, 'CA' => $sopacity, 'BM' => '/Normal']);
                $this->mpdf_ref->extgstates[$gs]['fo'] = true;
                $opacitystr = sprintf(' /GS%d gs ', $gs);

                $fillstr = '';
                if (isset($current_style['fill']) && 'none' != $current_style['fill']) {
                ...
                $strokestr = '';
                if ($stroke_width > 0 && 'none' != $current_style['stroke']) {
                ...
                while ($i < $nb) {
                    ...
                    foreach ($commands as $cmd) {
                        if (3 == count($cmd) || (isset($cmd[2]) && '' == $cmd[2])) {
                            [$tmp, $command, $arguments] = $cmd;
                        } else {
                            [$tmp, $command] = $cmd;
                            $arguments = '';
                        }
                    ...
                    $subpath_cmd .= $op.' Q ';
                    if (-1999998 == $this->pathBBox[2]) {
                        $this->pathBBox[2] = 100;
                    }
                    if (-1999998 == $this->pathBBox[3]) {
                        $this->pathBBox[3] = 100;
                    }
                    if (999999 == $this->pathBBox[0]) {
                        $this->pathBBox[0] = 0;
                    }
                    if (999999 == $this->pathBBox[1]) {
                        $this->pathBBox[1] = 0;
                    }
            ...
            // select font
            $style .= ('bold' == $current_style['font-weight']) ? 'B' : '';
            $style .= ('italic' == $current_style['font-style']) ? 'I' : '';
            ...
            $strokestr = '';
            if (isset($current_style['stroke-width']) && $current_style['stroke-width'] > 0 && 'none' != $current_style['stroke']) {
                $scol = $this->mpdf_ref->ConvertColor($current_style['stroke']);
                if ($scol) {
                    $strokestr .= $this->mpdf_ref->SetDColor($scol, true).' ';
                }
        ...
        $style = '';
        $style .= ('bold' == $prev_style['font-weight']) ? 'B' : '';
        $style .= ('italic' == $prev_style['font-style']) ? 'I' : '';
    ...
    public function svgDefineTxtStyle($critere_style): void
    {
        ...
        if (isset($critere_style['font-style']) && 'inherit' != $critere_style['font-style']) {
            if ('oblique' == strtolower($critere_style['font-style'])) {
                $critere_style['font-style'] = 'italic';
            }
            $current_style['font-style'] = $critere_style['font-style'];
        }

        if (isset($critere_style['font-weight']) && 'inherit' != $critere_style['font-weight']) {
            if ('bolder' == strtolower($critere_style['font-weight'])) {
                $critere_style['font-weight'] = 'bold';
            }
            $current_style['font-weight'] = $critere_style['font-weight'];
        }

        if (isset($critere_style['font-variant']) && 'inherit' != $critere_style['font-variant']) {
            $current_style['font-variant'] = $critere_style['font-variant'];
        }

        if (isset($critere_style['font-size']) && 'inherit' != $critere_style['font-size']) {
            if (str_contains($critere_style['font-size'], '%')) {
        ...
        if (isset($critere_style['font-family']) && 'inherit' != $critere_style['font-family']) {
            $v = $critere_style['font-family'];
            $aux_fontlist = explode(',', $v);
            $found = 0;

            $svgfontstyle = 'R';
            $svgfontstyle .= (isset($current_style['font-weight']) && 'bold' == $current_style['font-weight']) ? 'B' : '';
            $svgfontstyle .= (isset($current_style['font-style']) && 'italic' == $current_style['font-style']) ? 'I' : '';
            $svgfontstyle .= (isset($current_style['font-variant']) && 'small-caps' == $current_style['font-variant']) ? 'S' : '';
        ...
        if (isset($critere_style['text-anchor']) && 'inherit' != $critere_style['text-anchor']) {
    ...
    public function svgAddGradient($id, $array_gradient): void
    {
    ...
    public function svgWriteString($content): void
    {
    ...
    public function ImageSVG($data)
    {
        ...
        if (!function_exists('xml_svg2pdf_start')) {
            function xml_svg2pdf_start($parser, $name, $attribs): void
            {
                ...
                // mPDF 6
                if ('font-face' == strtolower($name)) {
                    $last_svg_fontstyle = 'R';
                    $last_svg_fontstyle .= (isset($attribs['font-weight']) && 'bold' == $attribs['font-weight']) ? 'B' : '';
                    $last_svg_fontstyle .= (isset($attribs['font-style']) && 'italic' == $attribs['font-style']) ? 'I' : '';
                    $last_svg_fontstyle .= (isset($attribs['font-variant']) && 'small-caps' == $attribs['font-variant']) ? 'S' : '';

                    if (isset($attribs['font-family']) && $attribs['font-family']) {
                        $tmp_svg_font = [
                            'units-per-em' => ($attribs['units-per-em'] ?? ''),
                            'd' => '',
                            'glyphs' => [],
                        ];
                ...
                // mPDF 6
                if ('missing-glyph' == strtolower($name)) {
                    if ($last_svg_fontid && isset($attribs['horiz-adv-x'])) {
                        $svg_class->svg_font[$last_svg_fontid][$last_svg_fontstyle]['horiz-adv-x'] = ($attribs['horiz-adv-x'] ?? '');
                        $svg_class->svg_font[$last_svg_fontid][$last_svg_fontstyle]['d'] = ($attribs['d'] ?? '');
                    }

                    return;
                }
                // mPDF 6
                if ('glyph' == strtolower($name)) {
                    if ($last_svg_fontid && isset($attribs['unicode'])) {
                        $svg_class->svg_font[$last_svg_fontid][$last_svg_fontstyle]['glyphs'][$attribs['unicode']] = [
                            'horiz-adv-x' => ($attribs['horiz-adv-x'] ?? $last_svg_fontdefw),
                            'd' => ($attribs['d'] ?? ''),
                        ];
                    }
                ...
                // mPDF 5.7.2
                if ('lineargradient' == strtolower($name)) {
                    $tmp_gradient = [
                        'type' => 'linear',
                ...
                if ('radialgradient' == strtolower($name)) {
                    $tmp_gradient = [
                        'type' => 'radial',
                        'transform' => ($attribs['gradientTransform'] ?? ''),
                        'units' => ($attribs['gradientUnits'] ?? ''),
                ...
                switch (strtolower($name)) {
                    ...
                    case 'path':
                        ...
                        foreach ($commands as $c) {
                            if ((isset($c) && 3 == count($c)) || (isset($c[2]) && '' == $c[2])) {
                                [$tmp, $command, $arguments] = $c;
                            } else {
                                [$tmp, $command] = $c;
                                $arguments = '';
                            }
                        ...
                        if (-1999998 == $svg_class->pathBBox[2]) {
                            $svg_class->pathBBox[2] = 100;
                        }
                        if (-1999998 == $svg_class->pathBBox[3]) {
                            $svg_class->pathBBox[3] = 100;
                        }
                        if (999999 == $svg_class->pathBBox[0]) {
                            $svg_class->pathBBox[0] = 0;
                        }
                        if (999999 == $svg_class->pathBBox[1]) {
                    ...
                    case 'line':
                        $arguments = [];
                        $arguments[0] = ($attribs['x1'] ?? '');
                        $arguments[1] = ($attribs['y1'] ?? '');
                        $arguments[2] = ($attribs['x2'] ?? '');
                        $arguments[3] = ($attribs['y2'] ?? '');
                        ...
                        if (_SVG_AUTOFONT && isset($attribs['lang']) && $attribs['lang']) {
                            if (!$svg_class->mpdf_ref->usingCoreFont) {
                                if ($attribs['lang'] != $svg_class->mpdf_ref->default_lang) {
                                    [$coreSuitable, $mpdf_unifont] = GetLangOpts($attribs['lang'], $svg_class->mpdf_ref->useAdobeCJK, $svg_class->mpdf_ref->fontdata);
                                    if ($mpdf_unifont) {
                    ...
                    case 'tspan':
                        ...
                        if (isset($attribs['x']) || isset($attribs['y'])) {
                            ...
                            if ('end' == $svg_class->textanchor) {
                                $tx = -$svg_class->texttotallength;
                            } elseif ('middle' == $svg_class->textanchor) {
                ...
                if (isset($path_cmd) && $path_cmd) {
                    // mPDF 5.0
                    [$prestyle, $poststyle] = $svg_class->svgStyle($path_style, $attribs, strtolower($name));
            ...
            function xml_svg2pdf_end($parser, $name): void
            {
                global $svg_class;
                // mPDF 5.7.2
                // Don't output stuff inside <defs>
            ...
            if (0 == $i % 2) {
                ...
                for ($sch = 0; $sch < $subchunk; ++$sch) {
                    if ($scriptblocks[$sch] > 0 && $scriptblocks[$sch] != $this->mpdf_ref->baseScript && $scriptblocks[$sch + 1] == $this->mpdf_ref->baseScript) {
                        ...
                        while (0 == $chardata[$sch][$end]['script'] && $end > 1) { // common script
                ...
                for ($sch = 0; $sch <= $subchunk; ++$sch) {
                    if (isset($chardata[$sch])) {
                        ...
                        if ('<text' != substr($a[$i - 1], 0, 5) && '<tspa' != substr($a[$i - 1], 0, 5)) {
                            continue;
                        } // <tspan> or <text> only

                        $lang = '';
                        // Check Vietnamese if Latin script - even if Basescript
                        if (UCDN::SCRIPT_LATIN == $scriptblocks[$sch] && $this->mpdf_ref->autoVietnamese && preg_match('/(['.$this->viet.'])/u', $s)) {
                            $lang = 'vi';
                        }
                        // Check Arabic for different languages if Arabic script - even if Basescript
                        elseif (UCDN::SCRIPT_ARABIC == $scriptblocks[$sch] && $this->mpdf_ref->autoArabic) {
                            if (preg_match('/['.$this->sindhi.']/u', $s)) {
                                ...
                                $lang = 'fa';
                            } elseif (UCDN::SCRIPT_ARABIC != $this->mpdf_ref->baseScript && isset($this->script2lang[$scriptblocks[$sch]])) {
