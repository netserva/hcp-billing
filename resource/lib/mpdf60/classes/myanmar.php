...
class MYANMAR
{
    ...
    // myanmar_category
    public const OT_DB = 3;  // same as INDIC::OT_N; /* Dot below */
    public const OT_GB = 12;  // same as INDIC::OT_DOTTEDCIRCLE;
    public const OT_As = 18; // Asat
    public const OT_D = 19; // Digits except zero
    public const OT_D0 = 20; // Digit zero
    public const OT_MH = 21; // Various consonant medial types
    public const OT_MR = 22; // Various consonant medial types
    public const OT_MW = 23; // Various consonant medial types
    public const OT_MY = 24; // Various consonant medial types
    public const OT_PT = 25; // Pwo and other tones

    public const OT_VAbv = 26;
    public const OT_VBlw = 27;
    public const OT_VPre = 28;
    public const OT_VPst = 29;
    public const OT_VS = 30; // Variation selectors
    ...
    public const NON_MYANMAR_CLUSTER = 4;
    ...
    public static function set_myanmar_properties(&$info): void
    {
        $u = $info['uni'];
    ...
    public static function reordering(&$info, $GSUBdata, $broken_syllables, $dottedcircle): void
    {
    ...
    public static function reordering_syllable(&$info, $GSUBdata, $start, $end): void
    {
        ...
        if (self::NON_MYANMAR_CLUSTER == $syllable_type) {
            return;
        }
        if (self::BROKEN_CLUSTER == $syllable_type) {
            ...
            if (self::OT_DOTTEDCIRCLE == $info[$end - 1]['myanmar_category']) {
                return;
            }
        }
        ...
        if (($start + 3 <= $end)
            && self::OT_Ra == $info[$start]['myanmar_category']
            && self::OT_As == $info[$start + 1]['myanmar_category']
            && self::OT_H == $info[$start + 2]['myanmar_category']) {
            $limit += 3;
            $base = $start;
            $has_reph = true;
        }
         ...
         * Myanmar reordering! */
        for (; $i < $end; ++$i) {
            if (self::OT_MR == $info[$i]['myanmar_category']) /* Pre-base reordering */ {
                $info[$i]['myanmar_position'] = self::POS_PRE_C;
            ...
            if (self::POS_AFTER_MAIN == $pos && self::OT_VBlw == $info[$i]['myanmar_category']) {
            ...
            if (self::POS_BELOW_C == $pos && self::OT_A == $info[$i]['myanmar_category']) {
                $info[$i]['myanmar_position'] = self::POS_BEFORE_SUB;

                continue;
            }
            if (self::POS_BELOW_C == $pos && self::OT_VBlw == $info[$i]['myanmar_category']) {
                $info[$i]['myanmar_position'] = $pos;
    ...
    public static function FLAG_RANGE($x, $y): void
    {
