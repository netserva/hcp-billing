...
class tocontents
{
    ...
    { // mPDF 5.6.19	// mPDF 6
        ...
        if (false === $TOCusePaging || 'off' == strtolower($TOCusePaging) || 0 === $TOCusePaging || '0' === $TOCusePaging || '' === $TOCusePaging) {
            $TOCusePaging = false;
        } else {
            $TOCusePaging = true;
        }
    ...
    public function TOC($tocfont = '', $tocfontsize = 0, $tocindent = 0, $resetpagenum = '', $pagenumstyle = '', $suppress = '', $toc_orientation = '', $TOCusePaging = true, $TOCuseLinking = false, $toc_id = 0, $tocoutdent = '', $toc_resetpagenum = '', $toc_pagenumstyle = '', $toc_suppress = ''): void
    { // mPDF 5.6.19	// mPDF 6
        if ('ALL' == strtoupper($toc_id)) {
            $toc_id = '_mpdf_all';
        } elseif (!$toc_id) {
    ...
    public function insertTOC(): void
    {
        $notocs = 0;
        if ($this->TOCmark) {
            $notocs = 1;
        ...
        for ($toci = 0; $toci < $notocs; ++$toci) {
            ...
            if ($toc_suppress || '0' === $toc_suppress) {
            ...
            foreach ($this->_toc as $t) {
                if ('_mpdf_all' === $t['toc_id'] || $t['toc_id'] === $toc_id) {
    ...
    public function openTagTOC($attr): void
    {
        ...
        if (isset($attr['PAGING']) && ('OFF' == strtoupper($attr['PAGING']) || '0' === $attr['PAGING'])) {
            $paging = false;
        } else {
            $paging = true;
        }
    ...
    public function openTagTOCPAGEBREAK($attr)
    {
        ...
        if ($toc_id) {
            ...
            if (isset($attr['TOC-ODD-HEADER-VALUE']) && ('1' == $attr['TOC-ODD-HEADER-VALUE'] || 'ON' == strtoupper($attr['TOC-ODD-HEADER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_odd_header_value'] = 1;
            } elseif (isset($attr['TOC-ODD-HEADER-VALUE']) && ('-1' == $attr['TOC-ODD-HEADER-VALUE'] || 'OFF' == strtoupper($attr['TOC-ODD-HEADER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_odd_header_value'] = -1;
            }
            if (isset($attr['TOC-EVEN-HEADER-VALUE']) && ('1' == $attr['TOC-EVEN-HEADER-VALUE'] || 'ON' == strtoupper($attr['TOC-EVEN-HEADER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_even_header_value'] = 1;
            } elseif (isset($attr['TOC-EVEN-HEADER-VALUE']) && ('-1' == $attr['TOC-EVEN-HEADER-VALUE'] || 'OFF' == strtoupper($attr['TOC-EVEN-HEADER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_even_header_value'] = -1;
            }
            if (isset($attr['TOC-ODD-FOOTER-VALUE']) && ('1' == $attr['TOC-ODD-FOOTER-VALUE'] || 'ON' == strtoupper($attr['TOC-ODD-FOOTER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_odd_footer_value'] = 1;
            } elseif (isset($attr['TOC-ODD-FOOTER-VALUE']) && ('-1' == $attr['TOC-ODD-FOOTER-VALUE'] || 'OFF' == strtoupper($attr['TOC-ODD-FOOTER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_odd_footer_value'] = -1;
            }
            if (isset($attr['TOC-EVEN-FOOTER-VALUE']) && ('1' == $attr['TOC-EVEN-FOOTER-VALUE'] || 'ON' == strtoupper($attr['TOC-EVEN-FOOTER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_even_footer_value'] = 1;
            } elseif (isset($attr['TOC-EVEN-FOOTER-VALUE']) && ('-1' == $attr['TOC-EVEN-FOOTER-VALUE'] || 'OFF' == strtoupper($attr['TOC-EVEN-FOOTER-VALUE']))) {
                $this->m_TOC[$toc_id]['TOC_even_footer_value'] = -1;
            }
            ...
            if (isset($attr['TOC-SUPPRESS']) && ($attr['TOC-SUPPRESS'] || '0' === $attr['TOC-SUPPRESS'])) {
        ...
        if ($this->mpdf->y == $this->mpdf->tMargin && (!$this->mpdf->mirrorMargins || ($this->mpdf->mirrorMargins && 1 == $this->mpdf->page % 2))) {
            if ($toc_id) {
                $this->m_TOC[$toc_id]['TOCmark'] = $this->mpdf->page;
            } else {
                $this->TOCmark = $this->mpdf->page;
            ...
            // Don't add a page
            if (1 == $this->mpdf->page && 0 == count($this->mpdf->PageNumSubstitutions)) {
                $resetpagenum = '';
                $pagenumstyle = '';
                $suppress = '';
