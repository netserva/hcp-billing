...
class INDIC
{
    ...
    public const OT_M = 7; // Matra or Dependent Vowel
    public const OT_SM = 8;
    public const OT_VD = 9;
    public const OT_A = 10;
    public const OT_NBSP = 11;
    public const OT_DOTTEDCIRCLE = 12; // Not in the spec, but special in Uniscribe. /Very very/ special!
    public const OT_RS = 13; // Register Shifter, used in Khmer OT spec
    public const OT_Coeng = 14;
    public const OT_Repha = 15;

    public const OT_Ra = 16; // Not explicitly listed in the OT spec, but used in the grammar.
    public const OT_CM = 17;
    ...
    public const _NUKT = 0;
    public const _AKHN = 1;
    public const RPHF = 2;
    public const _RKRF = 3;
    public const PREF = 4;
    public const BLWF = 5;
    public const HALF = 6;
    public const ABVF = 7;
    public const PSTF = 8;
    public const CFAR = 9; // Khmer only
    public const _VATU = 10;
    public const _CJCT = 11;
    public const INIT = 12;

    // syllable_type
    public const CONSONANT_SYLLABLE = 0;
    public const VOWEL_SYLLABLE = 1;
    public const STANDALONE_CLUSTER = 2;
    public const BROKEN_CLUSTER = 3;
    public const NON_INDIC_CLUSTER = 4;
    ...
    // base_position
    public const BASE_POS_FIRST = 0;
    public const BASE_POS_LAST = 1;

    // reph_position
    public const REPH_POS_DEFAULT = 10; // POS_BEFORE_POST,

    public const REPH_POS_AFTER_MAIN = 5; // POS_AFTER_MAIN,

    public const REPH_POS_BEFORE_SUB = 7; // POS_BEFORE_SUB,
    public const REPH_POS_AFTER_SUB = 9; // POS_AFTER_SUB,
    public const REPH_POS_BEFORE_POST = 10; // POS_BEFORE_POST,
    public const REPH_POS_AFTER_POST = 12; // POS_AFTER_POST

    // reph_mode
    public const REPH_MODE_IMPLICIT = 0;  // Reph formed out of initial Ra,H sequence.
    public const REPH_MODE_EXPLICIT = 1;  // Reph formed out of initial Ra,H,ZWJ sequence.
    public const REPH_MODE_VIS_REPHA = 2; // Encoded Repha character, no reordering needed.
    public const REPH_MODE_LOG_REPHA = 3; // Encoded Repha character, needs reordering.
    ...
    public static function set_indic_properties(&$info, $scriptblock): void
    {
        $u = $info['uni'];
        ...
        if (self::OT_X == $cat && self::in_range($u, 0x17CB, 0x17D3)) { // Khmer Various signs
        ...
        if (self::OT_Repha == $cat) {
             ...
             */
            if (UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK == $info['general_category']) {
                $cat = self::OT_N;
            }
        }
        ...
        if ((self::FLAG($cat) & (self::FLAG(self::OT_C) | self::FLAG(self::OT_CM) | self::FLAG(self::OT_Ra) | self::FLAG(self::OT_V) | self::FLAG(self::OT_NBSP) | self::FLAG(self::OT_DOTTEDCIRCLE)))) { // = CONSONANT_FLAGS like is_consonant
            if (UCDN::SCRIPT_KHMER == $scriptblock) {
        ...
        } elseif (self::OT_M == $cat) {
            $pos = self::matra_position($u, $pos);
        } elseif (self::OT_SM == $cat || self::OT_VD == $cat) {
            $pos = self::POS_SMVD;
        }
    ...
    public static function set_syllables_sinhala(&$o, $s, &$broken_syllables): void
    {
        $ptr = 0;
        $syllable_serial = 1;
        $broken_syllables = false;
    ...
    public static function set_syllables_khmer(&$o, $s, &$broken_syllables): void
    {
        $ptr = 0;
        $syllable_serial = 1;
        $broken_syllables = false;
    ...
    public static function initial_reordering(&$info, $GSUBdata, $broken_syllables, $indic_config, $scriptblock, $is_old_spec, $dottedcircle): void
    {
    ...
    public static function update_consonant_positions(&$info, $GSUBdata): void
    {
        $count = count($info);
        for ($i = 0; $i < $count; ++$i) {
            if (self::POS_BASE_C == $info[$i]['indic_position']) {
                $c = $info[$i]['uni'];
                // If would substitute...
    ...
    public static function insert_dotted_circles(&$info, $dottedcircle): void
    {
        ...
        while ($idx < count($info)) {
            ...
            if ($last_syllable != $syllable && self::BROKEN_CLUSTER == $syllable_type) {
                ...
                while ($idx < count($info) && $last_syllable == $info[$idx]['syllable'] && self::OT_Repha == $info[$idx]['indic_category']) {
                    ++$idx;
                }
    ...
    public static function initial_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end): void
    {
        ...
        if (self::NON_INDIC_CLUSTER == $syllable_type) {
            return;
        }
        if (self::BROKEN_CLUSTER == $syllable_type || self::STANDALONE_CLUSTER == $syllable_type) {
            //if ($uniscribe_bug_compatible) {
            /* For dotted-circle, this is what Uniscribe does:
             * If dotted-circle is the last glyph, it just does nothing.
             * i.e. It doesn't form Reph. */
            if (self::OT_DOTTEDCIRCLE == $info[$end - 1]['indic_category']) {
                return;
            }
        }
        ...
        if (UCDN::SCRIPT_KHMER != $scriptblock) {
            ...
            if (count($GSUBdata['rphf']) /* ?? $indic_plan->mask_array[RPHF] */ && $start + 3 <= $end
                && (
                    (self::REPH_MODE_IMPLICIT == $indic_config[4] && !self::is_joiner($info[$start + 2]))
                || (self::REPH_MODE_EXPLICIT == $indic_config[4] && self::OT_ZWJ == $info[$start + 2]['indic_category'])
                )) {
            ...
            } elseif (self::REPH_MODE_LOG_REPHA == $indic_config[4] && self::OT_Repha == $info[$start]['indic_category']) {
                ++$limit;
                while ($limit < $end && self::is_joiner($info[$limit])) {
                    ++$limit;
                }
        ...
        switch ($indic_config[2]) { // base_pos
            case self::BASE_POS_LAST:
                ...
                do {
                    ...
                    if (self::is_consonant($info[$i])) {
                        ...
                        if (self::POS_BELOW_C != $info[$i]['indic_position'] && (self::POS_POST_C != $info[$i]['indic_position'] || $seen_below)) {
                            $base = $i;

                            break;
                        }
                        if (self::POS_BELOW_C == $info[$i]['indic_position']) {
                            $seen_below = true;
                        }
                        ...
                        if ($start < $i && self::OT_ZWJ == $info[$i]['indic_category'] && self::OT_H == $info[$i - 1]['indic_category']) {
                        ...
                        // ZKI8
                        if ($start < $i && self::OT_ZWNJ == $info[$i]['indic_category']) {
                            break;
                        }
                    }
                } while ($i > $limit);
            ...
            case self::BASE_POS_FIRST:
                ...
                for ($i = $limit; $i < $end; ++$i) {
                    if (self::is_consonant($info[$i]) && self::POS_BASE_C == $info[$i]['indic_position']) {
                        if ($limit < $i && self::OT_ZWJ == $info[$i - 1]['indic_category']) {
                            break;
                        }
         ...
         * ? only in Khmer. */
        for ($i = $base + 1; $i < $end; ++$i) {
            if (self::OT_M == $info[$i]['indic_category']) {
                for ($j = $i + 1; $j < $end; ++$j) {
        ...
        if ($is_old_spec) {
            for ($i = $base + 1; $i < $end; ++$i) {
                if (self::OT_H == $info[$i]['indic_category']) {
                    for ($j = $end - 1; $j > $i; --$j) {
                        if (self::is_consonant($info[$j]) || self::OT_H == $info[$j]['indic_category']) {
                            break;
                        }
                    }
                    if (self::OT_H != $info[$j]['indic_category'] && $j > $i) {
        ...
        for ($i = $start; $i < $end; ++$i) {
            if ((self::FLAG($info[$i]['indic_category']) & (self::FLAG(self::OT_ZWJ) | self::FLAG(self::OT_ZWNJ) | self::FLAG(self::OT_N) | self::FLAG(self::OT_RS) | self::FLAG(self::OT_H) | self::FLAG(self::OT_Coeng)))) {
                ...
                if (self::OT_H == $info[$i]['indic_category'] && self::POS_PRE_M == $info[$i]['indic_position']) {
                    /*
                     ...
                     */
                    for ($j = $i; $j > $start; --$j) {
                        if (self::POS_PRE_M != $info[$j - 1]['indic_position']) {
            ...
            } elseif (self::POS_SMVD != $info[$i]['indic_position']) {
        ...
        for ($i = $base + 1; $i < $end; ++$i) {
            if (self::is_halant_or_coeng($info[$i])) {
                ...
                for ($j = $last_halant; $j < $i; ++$j) {
                    if (self::POS_SMVD != $info[$j]['indic_position']) {
        ...
        if (UCDN::SCRIPT_KHMER != $scriptblock) {
            // Reph
            for ($i = $start; $i < $end; ++$i) {
                if (self::POS_RA_TO_BECOME_REPH == $info[$i]['indic_position']) {
        ...
        if ($is_old_spec && UCDN::SCRIPT_DEVANAGARI == $scriptblock) {
            ...
            for ($i = $start; ($i + 1) < $base; ++$i) {
                if (self::OT_Ra == $info[$i]['indic_category'] && self::OT_H == $info[$i + 1]['indic_category']
                    && ($i + 2 == $base || self::OT_ZWJ != $info[$i + 2]['indic_category'])) {
        ...
        for ($i = $start + 1; $i < $end; ++$i) {
            if (self::is_joiner($info[$i])) {
                $non_joiner = (self::OT_ZWNJ == $info[$i]['indic_category']);
                $j = $i;
                while ($j > $start) {
    ...
    public static function final_reordering(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec): void
    {
        $count = count($info);
        if (!$count) {
            return;
    ...
    public static function final_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end): void
    {
        /* 4. Final reordering:
         *
        ...
        if ($base == $end && $start < $base && self::OT_ZWJ != $info[$base - 1]['indic_category']) {
            --$base;
        }
        while ($start < $base && isset($info[$base]) && (self::OT_H == $info[$base]['indic_category'] || self::OT_N == $info[$base]['indic_category'])) {
            --$base;
        }
        ...
        if ($start + 1 < $end && $start < $base) { // Otherwise there can't be any pre-base matra characters.
             ...
             */
            if (UCDN::SCRIPT_MALAYALAM != $scriptblock && UCDN::SCRIPT_TAMIL != $scriptblock) {
                ...
                if (self::is_halant_or_coeng($info[$new_pos]) && self::POS_PRE_M != $info[$new_pos]['indic_position']) {
            ...
            if ($start < $new_pos && self::POS_PRE_M != $info[$new_pos]['indic_position']) {
                // Now go see if there's actually any matras...
                for ($i = $new_pos; $i > $start; --$i) {
                    if (self::POS_PRE_M == $info[$i - 1]['indic_position']) {
                        $old_pos = $i - 1;
         ...
         * shouldn't move. */
        if ($start + 1 < $end
            && self::POS_RA_TO_BECOME_REPH == $info[$start]['indic_position'] && self::POS_RA_TO_BECOME_REPH != $info[$start + 1]['indic_position']) {
             ...
             * 	proceed to step 5.
             */
            if (self::REPH_POS_AFTER_POST == $reph_pos) {
             ...
             */
            if (self::REPH_POS_AFTER_MAIN == $reph_pos && !$skip_to_reph_move && !$skip_to_reph_step_5) {
                $new_reph_pos = $base;
            ...
            if (self::REPH_POS_AFTER_SUB == $reph_pos && !$skip_to_reph_move && !$skip_to_reph_step_5) {
                $new_reph_pos = $base;
            ...
            if (!$skip_to_reph_move) {
                $new_reph_pos = $end - 1;
                while ($new_reph_pos > $start && self::POS_SMVD == $info[$new_reph_pos]['indic_position']) {
                    --$new_reph_pos;
                }
                ...
                if ($info[$i]['mask'] & self::FLAG(self::PREF)) {
                    ...
                    if ($i + 1 == $end || ($info[$i + 1]['mask'] & self::FLAG(self::PREF)) == 0) {
                        ...
                        if (UCDN::SCRIPT_MALAYALAM != $scriptblock && UCDN::SCRIPT_TAMIL != $scriptblock) {
                            ...
                            if ($new_pos > $start && self::OT_M == $info[$new_pos - 1]['indic_category']) {
                                $old_pos = i;
        ...
        if (self::POS_PRE_M == $info[$start]['indic_position']
            && (
                0 == $start
    ...
    public static function _move_info_pos(&$info, $from, $to): void
    {
        $t = [];
        $t[0] = $info[$from];
        if ($from > $to) {
