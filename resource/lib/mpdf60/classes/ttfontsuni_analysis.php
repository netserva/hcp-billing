...
class TTFontFile_Analysis extends TTFontFile
{
    ...
    public function extractCoreInfo($file, $TTCfontID = 0)
    {
        ...
        $name_offset = $this->seek_table('name');
        $format = $this->read_ushort();
        if (0 != $format && 1 != $format) { // mPDF 5.3.73
        ...
        for ($i = 0; $i < $cmapTableCount; ++$i) {
            ...
            } elseif (((3 == $platformID && 10 == $encodingID) || 0 == $platformID)) { // Microsoft, Unicode Format 12 table HKCS
                $format = $this->get_ushort($cmap_offset + $offset);
                if (12 == $format) {
                    $unicode_cmap_offset = $cmap_offset + $offset;
        ...
        if ($panose) {
            ...
            if (2 == $bFamilyType) {
                ...
                if (9 == $bProportion || 1 == $bProportion) {
                    $ftype = 'mono';
                } // ==1 i.e. No Fit needed for OCR-a and -b
            } elseif (3 == $bFamilyType) {
                $ftype = 'cursive';
            }
        }
