...
class mpdfform
{
    ...
    public function print_ob_text($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir): void
    {
        // TEXT/PASSWORD INPUT
        if ($this->mpdf->useActiveForms) {
            ...
            if (isset($objattr['subtype']) && 'PASSWORD' == $objattr['subtype']) {
                $flags[] = 14;
                $val = $objattr['value'];
            }
            ...
            $this->SetFormText($w, $h, $objattr['fieldname'], $val, $val, $objattr['title'], $flags, $fieldalign, false, ($objattr['maxlength'] ?? false), $js, ($objattr['background-col'] ?? false), ($objattr['border-col'] ?? false));
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        } else {
    ...
    public function print_ob_textarea($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir): void
    {
        // TEXTAREA
        if ($this->mpdf->useActiveForms) {
            // Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea; 14 - Password
            ...
            $this->SetFormText($w, $h, $objattr['fieldname'], $texto, $texto, ($objattr['title'] ?? ''), $flags, $fieldalign, false, -1, $js, ($objattr['background-col'] ?? false), ($objattr['border-col'] ?? false));
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        } else {
    ...
    public function print_ob_select($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir): void
    {
        // SELECT
        if ($this->mpdf->useActiveForms) {
            ...
            if (0 == count($data['SEL']) && $this->formSelectDefaultOption) {
                $data['SEL'][] = 0;
            }
            if (isset($objattr['color'])) {
                $this->mpdf->SetTColor($objattr['color']);
    ...
    public function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir): void
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->SetJSButton($w, $h, $objattr['fieldname'], ($objattr['value'] ?? ''), $js, $objattr['ID'], $objattr['title'], $flags, ($objattr['Indexed'] ?? false));
        } else {
    ...
    public function print_ob_button($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir): void
    {
        // BUTTON
        if ($this->mpdf->useActiveForms) {
            // Flags: 1 - Readonly; 3 - No export;
            ...
            if (isset($objattr['subtype']) && 'RESET' == $objattr['subtype']) {
                $this->SetFormButtonText($objattr['value']);
                $this->SetFormReset($w, $h, $objattr['fieldname'], $objattr['value'], $objattr['title'], $flags, ($objattr['background-col'] ?? false), ($objattr['border-col'] ?? false), ($objattr['noprint'] ?? false));
            } elseif (isset($objattr['subtype']) && 'SUBMIT' == $objattr['subtype']) {
                $url = $this->formAction;
                $type = $this->formExportType;
                $method = $this->formMethod;
                $this->SetFormButtonText($objattr['value']);
                $this->SetFormSubmit($w, $h, $objattr['fieldname'], $objattr['value'], $url, $objattr['title'], $type, $method, $flags, ($objattr['background-col'] ?? false), ($objattr['border-col'] ?? false), ($objattr['noprint'] ?? false));
            } elseif (isset($objattr['subtype']) && 'BUTTON' == $objattr['subtype']) {
                $this->SetFormButtonText($objattr['value']);
                if (isset($objattr['onClick']) && $objattr['onClick']) {
                    $js = $objattr['onClick'];
                } else {
                ...
                $this->SetJSButton($w, $h, $objattr['fieldname'], $objattr['value'], $js, 0, $objattr['title'], $flags, false, ($objattr['background-col'] ?? false), ($objattr['border-col'] ?? false), ($objattr['noprint'] ?? false));
            }
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        } else {
            $this->mpdf->SetLineWidth(0.2 / $k);
    ...
    public function print_ob_checkbox($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $x, $y): void
    {
        // CHECKBOX
        if ($this->mpdf->useActiveForms) {
            ...
            $this->SetCheckBox($w, $h, $objattr['fieldname'], $objattr['value'], $objattr['title'], $checked, $flags, ($objattr['disabled'] ?? false));
            if ($this->formUseZapD) {
                $this->mpdf->SetFont($save_font, '', 0);
                $this->mpdf->currentfontfamily = $save_currentfont;
            }
    ...
    public function print_ob_radio($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $x, $y): void
    {
        // RADIO
        if ($this->mpdf->useActiveForms) {
            // Flags: 1 - Readonly; 2 - Required; 3 - No export;
            ...
            $this->SetRadio($w, $h, $objattr['fieldname'], $objattr['value'], ($objattr['title'] ?? ''), $checked, $flags, ($objattr['disabled'] ?? false));
            if ($this->formUseZapD) {
                $this->mpdf->SetFont($save_font, '', 0);
                $this->mpdf->currentfontfamily = $save_currentfont;
            }
    ...
    // In _putpages
    public function countPageForms($n, &$totaladdnum): void
    {
        foreach ($this->forms as $form) {
            if ($form['page'] == $n) {
                ...
                if ('Bt' == $form['typ']) {
                    ...
                    if ('radio' == $form['subtype']) {
                        $totaladdnum += 2;
                    } elseif ('checkbox' == $form['subtype'] && $this->formUseZapD) {
                        ++$totaladdnum;
                    } elseif ('checkbox' == $form['subtype'] && !$this->formUseZapD) {
                        $totaladdnum += 2;
                    }
    ...
    // In _putpages
    public function addFormIds($n, &$s, &$annotid): void
    {
        foreach ($this->forms as $form) {
            if ($form['page'] == $n) {
                $s .= ($annotid).' 0 R ';
    ...
    // In _putannots
    public function _putFormItems($n, $hPt): void
    {
    ...
    // In _putannots
    public function _putRadioItems($n): void
    {
        // Output Radio Groups
        $key = 1;
    ...
    public function _putFormsCatalog(): void
    {
    ...
    public function SetFormButtonJS($name, $js): void
    {
    ...
    public function SetFormChoiceJS($name, $js): void
    {
        $js = str_replace("\t", ' ', trim($js));
        if (isset($name, $js)) {
    ...
    public function SetFormTextJS($name, $js): void
    {
    ...
    public function Win1252ToPDFDocEncoding($txt)
    {
        $Win1252ToPDFDocEncoding = [
            chr(0o200) => chr(0o240), chr(0o214) => chr(0o226), chr(0o212) => chr(0o227), chr(0o237) => chr(0o230),
            chr(0o225) => chr(0o200), chr(0o210) => chr(0o032), chr(0o206) => chr(0o201), chr(0o207) => chr(0o202),
            chr(0o205) => chr(0o203), chr(0o227) => chr(0o204), chr(0o226) => chr(0o205), chr(0o203) => chr(0o206),
            chr(0o213) => chr(0o210), chr(0o233) => chr(0o211), chr(0o211) => chr(0o213), chr(0o204) => chr(0o214),
            chr(0o223) => chr(0o215), chr(0o224) => chr(0o216), chr(0o221) => chr(0o217), chr(0o222) => chr(0o220),
            chr(0o202) => chr(0o221), chr(0o232) => chr(0o235), chr(0o230) => chr(0o037), chr(0o231) => chr(0o222),
            chr(0o216) => chr(0o231), chr(0o240) => chr(0o040),
        ]; // mPDF 5.3.46

        return strtr($txt, $Win1252ToPDFDocEncoding);
    }

    public function SetFormText($w, $h, $name, $value = '', $default = '', $title = '', $flags = [], $align = 'L', $hidden = false, $maxlen = -1, $js = '', $background_col = false, $border_col = false): void
    {
        // Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea; 14 - Password
        ++$this->formn;
        if ('C' == $align) {
    ...
    public function SetFormChoice($w, $h, $name, $flags, $array, $align = 'L', $js = ''): void
    {
        ++$this->formn;
        if ('rtl' == $this->mpdf->blk[$this->mpdf->blklvl]['direction']) {
            $align = '2';
        } else {
            $align = '0';
        }
    ...
    // CHECKBOX
    public function SetCheckBox($w, $h, $name, $value, $title = '', $checked = false, $flags = [], $disabled = false): void
    {
    ...
    // RADIO
    public function SetRadio($w, $h, $name, $value, $title = '', $checked = false, $flags = [], $disabled = false): void
    {
        $this->SetFormButton($w, $h, $name, $value, 'radio', $title, $flags, $checked, $disabled);
        $this->mpdf->x += $w;
    }

    public function SetFormReset($w, $h, $name, $value = 'Reset', $title = '', $flags = [], $background_col = false, $border_col = false, $noprint = false): void
    {
        if (!$name) {
            $name = 'Reset';
        }
    ...
    public function SetJSButton($w, $h, $name, $value, $js, $image_id = 0, $title = '', $flags = [], $indexed = false, $background_col = false, $border_col = false, $noprint = false): void
    {
    ...
    public function SetFormSubmit($w, $h, $name, $value, $url, $title = '', $typ = 'html', $method = 'POST', $flags = [], $background_col = false, $border_col = false, $noprint = false): void
    {
        if (!$name) {
            $name = 'Submit';
        }
    ...
    public function SetFormButtonText($ca, $rc = '', $ac = ''): void
    {
        if ($this->mpdf->onlyCoreFonts) {
    ...
    public function SetFormButton($bb, $hh, $name, $value, $type, $title = '', $flags = [], $checked = false, $disabled = false, $background_col = false, $border_col = false, $noprint = false): void
    {
        ++$this->formn;
        if (!preg_match('/^[a-zA-Z0-9_:\-]+$/', $name)) {
            $this->mpdf->Error('Field ['.$name.'] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)');
    ...
    public function SetFormBorderWidth($string): void
    {
        switch ($string) {
    ...
    public function SetFormBorderStyle($string): void
    {
        switch ($string) {
    ...
    public function SetFormBorderColor($r, $g = -1, $b = -1): void
    {
        if ((0 == $r and 0 == $g and 0 == $b) || -1 == $g) {
    ...
    public function SetFormBackgroundColor($r, $g = -1, $b = -1): void
    {
        if ((0 == $r and 0 == $g and 0 == $b) || -1 == $g) {
    ...
    public function SetFormD($W, $S, $BC, $BG): void
    {
    ...
    public function _put_button_icon($array, $w, $h): void
    {
    ...
    public function _putform_bt($form, $hPt)
    {
        ...
        $this->mpdf->_out('/FT /Btn ');
        $this->mpdf->_out('/H /P ');
        if ('radio' != $form['subtype']) {  // mPDF 5.3.23
