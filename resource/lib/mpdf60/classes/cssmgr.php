...
class cssmgr
{
    ...
    public function ReadCSS($html)
    {
        ...
        if (count($idata[0])) {
            for ($i = 0; $i < count($idata[0]); ++$i) {
                $file = _MPDF_TEMP_PATH.'_tempCSSidata'.random_int(1, 10000).'_'.$i.'.'.$idata[2][$i];
                //Save to local file
            ...
            for ($i = 0; $i < count($styles[1]); ++$i) {
                ...
                foreach ($stylearr as $sta) {
                    if (trim($sta)) {
                        ...
                        if ($property && ($value || '0' === $value)) {
                            ...
                            if (('BACKGROUND-IMAGE' == strtoupper($property) || 'BACKGROUND' == strtoupper($property)) && preg_match('/-webkit-gradient/i', $value)) {
                                continue;
                            }
                ...
                foreach ($tagarr as $tg) {
                    ...
                    if ('@PAGE' == trim($tags[0])) {
                        ...
                        if (1 == $level) {
                            $tag = $t;
                        } elseif (2 == $level && preg_match('/^[:](.*)$/', $t2, $m)) {
                            $tag = $t.'>>PSEUDO>>'.$m[1];
                            ...
                            $tag = $t.'>>NAMED>>'.$t2;
                        } elseif (3 == $level && preg_match('/^[:](.*)$/', $t3, $m)) {
                    ...
                    } elseif (1 == $level) {  // e.g. p or .class or #id or p.class or p#id
                        if (isset($tags[0])) {
                            $t = trim($tags[0]);
                        }
                        if ($t) {
    ...
    public function readInlineCSS($html)
    {
        ...
        for ($i = 0; $i < count($properties); ++$i) {
            // Ignores -webkit-gradient so doesn't override -moz-
            if (('BACKGROUND-IMAGE' == strtoupper($properties[$i]) || 'BACKGROUND' == strtoupper($properties[$i])) && preg_match('/-webkit-gradient/i', $values[$i])) {
                continue;
            }
    ...
    public function _fix_borderStr($bd)
    {
        ...
        if (1 == count($prop)) {
            // solid
            if (in_array($prop[0], $this->mpdf->borderstyles) || 'none' == $prop[0] || 'hidden' == $prop[0]) {
                $s = $prop[0];
            }
            // #000000
            ...
            // 1px solid
            if (in_array($prop[1], $this->mpdf->borderstyles) || 'none' == $prop[1] || 'hidden' == $prop[1]) {
                $w = $prop[0];
                $s = $prop[1];
            }
            // solid #000000
            elseif (in_array($prop[0], $this->mpdf->borderstyles) || 'none' == $prop[0] || 'hidden' == $prop[0]) {
                $s = $prop[0];
                $c = $prop[1];
            }
            // 1px #000000
    ...
    {
        if (!is_array($prop) || (0 == count($prop))) {
            return [];
        }
        $newprop = [];
        foreach ($prop as $k => $v) {
            if ('BACKGROUND-IMAGE' != $k && 'BACKGROUND' != $k && 'ODD-HEADER-NAME' != $k && 'EVEN-HEADER-NAME' != $k && 'ODD-FOOTER-NAME' != $k && 'EVEN-FOOTER-NAME' != $k && 'HEADER' != $k && 'FOOTER' != $k) {
                $v = strtolower($v);
            }
            ...
            /* -- BORDER-RADIUS -- */ elseif ('BORDER-RADIUS' == $k || 'BORDER-TOP-LEFT-RADIUS' == $k || 'BORDER-TOP-RIGHT-RADIUS' == $k || 'BORDER-BOTTOM-LEFT-RADIUS' == $k || 'BORDER-BOTTOM-RIGHT-RADIUS' == $k) {
            ...
            } elseif ('TEXT-OUTLINE' == $k) { // mPDF 5.6.07
                $prop = preg_split('/\s+/', trim($v));
                if ('none' == trim(strtolower($v))) {
            ...
            /* -- BACKGROUNDS -- */ elseif ('BACKGROUND-IMAGE' == $k) {
            ...
            /* -- END BACKGROUNDS -- */ elseif ('IMAGE-ORIENTATION' == $k) {
                if (preg_match('/([\-]*[0-9\.]+)(deg|grad|rad)/i', $v, $m)) {
                    $angle = $m[1] + 0;
                    if ('deg' == strtolower($m[2])) {
                        $angle = $angle;
                    } elseif ('grad' == strtolower($m[2])) {
                        $angle *= (360 / 400);
                    } elseif ('rad' == strtolower($m[2])) {
                        $angle = rad2deg($angle);
                    }
                    while ($angle < 0) {
                        $angle += 360;
    ...
    public function border_radius_expand($val, $k)
    {
        ...
        if ('BORDER-TOP-LEFT-RADIUS' == $k) {
            $b['TL-H'] = $h;
            $b['TL-V'] = $v;
        } elseif ('BORDER-TOP-RIGHT-RADIUS' == $k) {
            $b['TR-H'] = $h;
            $b['TR-V'] = $v;
        } elseif ('BORDER-BOTTOM-LEFT-RADIUS' == $k) {
            $b['BL-H'] = $h;
            $b['BL-V'] = $v;
        } elseif ('BORDER-BOTTOM-RIGHT-RADIUS' == $k) {
            $b['BR-H'] = $h;
            $b['BR-V'] = $v;
        }
    ...
    public function _mergeCSS($p, &$t): void
    {
    ...
    public function _mergeFullCSS($p, &$t, $tag, $classes, $id, $lang): void
    { // mPDF 6
        if (isset($p[$tag])) {
        ...
        // STYLESHEET CLASS e.g. [lang=fr]{} or :lang(fr)
        if (isset($lang, $p[$tag.'>>LANG>>'.$lang])) {
            $this->_mergeCSS($p[$tag.'>>LANG>>'.$lang], $t);
        }
        // STYLESHEET CLASS e.g. #smallone{}  #redletter{}
        if (isset($id, $p[$tag.'>>ID>>'.$id])) {
            $this->_mergeCSS($p[$tag.'>>ID>>'.$id], $t);
        }
    }

    public function setBorderDominance($prop, $val): void
    {
    ...
    public function _set_mergedCSS(&$m, &$p, $d = true, $bd = false): void
    {
        if (isset($m)) {
            if ((isset($m['depth']) && $m['depth'] > 1) || false == $d) {  // include check for 'depth'
                if ($bd) {
    ...
    public function _mergeBorders(&$b, &$a): void
    ...
    public function MergeCSS($inherit, $tag, $attr)
    {
        ...
        //===============================================
        // Set Inherited properties
        if ('TOPTABLE' == $inherit || 'TABLE' == $inherit) {
        ...
        if ('BLOCK' == $inherit) {
            ...
            if (isset($this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) && $this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) {
                if ('L' == $this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) {
                    $p['TEXT-ALIGN'] = 'left';
                } elseif ('J' == $this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) {
                    $p['TEXT-ALIGN'] = 'justify';
                } elseif ('R' == $this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) {
                    $p['TEXT-ALIGN'] = 'right';
                } elseif ('C' == $this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) {
                    $p['TEXT-ALIGN'] = 'center';
                }
            }
            ...
            if (isset($this->mpdf->blk[$this->mpdf->blklvl - 1]['text_indent']) && ($this->mpdf->blk[$this->mpdf->blklvl - 1]['text_indent'] || 0 === $this->mpdf->blk[$this->mpdf->blklvl - 1]['text_indent'])) {
        ...
        if ('TABLE' == $tag && isset($attr['CELLSPACING'])) {
        ...
        if (('TD' == $tag || 'TH' == $tag) && isset($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['cell_padding']) && ($this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['cell_padding'] || $this->mpdf->table[$this->mpdf->tableLevel][$this->mpdf->tbctr[$this->mpdf->tableLevel]]['cell_padding'] === '0')) {  // mPDF 5.7.3
        ...
        if (isset($attr['ID'], $this->CSS['ID>>'.$attr['ID']]) && $this->CSS['ID>>'.$attr['ID']]) {
        ...
        //===============================================
        // STYLESHEET CLASS e.g. p#smallone{}  div#redletter{}
        if (isset($attr['ID'], $this->CSS[$tag.'>>ID>>'.$attr['ID']]) && $this->CSS[$tag.'>>ID>>'.$attr['ID']]) {
        ...
        /* -- TABLES -- */ elseif ('TOPTABLE' == $inherit || 'TABLE' == $inherit) { // NB looks at $this->tablecascadeCSS-1 for cascading CSS
    ...
    public function inlinePropsToCSS($bilp, &$p): void
    {
        ...
        if (isset($bilp['textparam']) && $bilp['textparam']) {
            if (isset($bilp['textparam']['hyphens'])) {
                if (2 == $bilp['textparam']['hyphens']) {
                    $p['HYPHENS'] = 'none';
                }
                if (1 == $bilp['textparam']['hyphens']) {
                    $p['HYPHENS'] = 'auto';
                }
                if (0 == $bilp['textparam']['hyphens']) {
                    $p['HYPHENS'] = 'manual';
                }
            }
    ...
    public function PreviewBlockCSS($tag, $attr)
    {
        ...
        // STYLESHEET ID e.g. #smallone{}  #redletter{}
        if (isset($attr['ID'], $this->CSS['ID>>'.$attr['ID']])) {
            $zp = $this->CSS['ID>>'.$attr['ID']];
            if (is_array($zp)) {
                $p = array_merge($p, $zp);
            }
        ...
        // STYLESHEET CLASS e.g. p#smallone{}  div#redletter{}
        if (isset($attr['ID'], $this->CSS[$tag.'>>ID>>'.$attr['ID']])) {
            $zp = $this->CSS[$tag.'>>ID>>'.$attr['ID']];
            if (is_array($zp)) {
                $p = array_merge($p, $zp);
            }
