...
class TTFontFile
{
    ...
    public function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = false, $BMPonly = false, $useOTL = 0): void
    { // mPDF 5.7.1
        ...
        if (0x74746366 == $version && !$TTCfontID) {
    ...
    public function readTableDirectory($debug = false): void
    {
    ...
    public function checksumTables(): void
    {
    ...
    { // mPDF 5.7.1
        ...
        for ($i = 0; $i < $cmapTableCount; ++$i) {
            ...
            } elseif (0 == $platformID) { // Unicode -- assume all encodings are compatible
                $format = $this->get_ushort($cmap_offset + $offset);
                if (4 == $format) {
                    $unicode_cmap_offset = $cmap_offset + $offset;
    ...
    public function extractInfo($debug = false, $BMPonly = false, $useOTL = 0): void
    {
        ...
        for ($i = 0; $i < $cmapTableCount; ++$i) {
            ...
            elseif (((3 == $platformID && 10 == $encodingID) || 0 == $platformID) && !$BMPonly) {
                $format = $this->get_ushort($cmap_offset + $offset);
                if (12 == $format) {
                    $unicode_cmap_offset = $cmap_offset + $offset;
        ...
        $format = $scoverage >> 8;
        if ($kern_offset && 0 == $version && 0 == $format) {
            // Format 0
            ...
            for ($i = 0; $i < $nPairs; ++$i) {
                ...
                $val = $this->read_short();
                if (1 == count($glyphToChar[$left]) && 1 == count($glyphToChar[$right])) {
                    if (32 != $left && 32 != $right) {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////
    public function _getGDEFtables(): void
    {
        ///////////////////////////////////
        ...
        if (isset($this->tables['GDEF'])) {
            ...
            if (2 == $ver_min && $MarkGlyphSetsDef_offset) {
        ...
        if (isset($this->tables['GSUB'])) {
            ...
            for ($i = 0; $i < $LookupCount; ++$i) {
                for ($c = 0; $c < $GSLookup[$i]['SubtableCount']; ++$c) {
                    ...
                    if (5 == $GSLookup[$i]['Type'] && 3 == $PosFormat) {
                        $this->skip(4);
                    } elseif (6 == $GSLookup[$i]['Type'] && 3 == $PosFormat) {
                ...
                for ($c = 0; $c < $Lookup[$i]['SubtableCount']; ++$c) {
                    ...
                    elseif (2 == $Lookup[$i]['Type']) {
                        ...
                        for ($g = 0; $g < count($glyphs); ++$g) {
                            ...
                            if (!isset($Lookup[$i]['Subtable'][$c]['Sequences'][$g]['SubstituteGlyphID']) || 0 == count($Lookup[$i]['Subtable'][$c]['Sequences'][$g]['SubstituteGlyphID'])) {
                                continue;
...
//print_r($langsys ); exit;
                    ...
                    $rtlSUB = 'array()';
                    $finals = '';
                    if (str_contains('arab syrc hebr thaa nko  samr', $st)) { // all RTL scripts [any/all languages] ? Mandaic
                        //print_r($volt); exit;
                        foreach ($volt as $v) {
                            // isol fina fin2 fin3 medi med2 for Syriac
                            // ISOLATED FORM :: FINAL :: INITIAL :: MEDIAL :: MED2 :: FIN2 :: FIN3
                            if (str_contains('isol fina init medi fin2 fin3 med2', $v['tag'])) {
                                $key = $v['match'];
                        ...
                        foreach ($rtl as $base => $variants) {
                            if (isset($variants[1])) { // i.e. final form
                                if (!str_contains('0FE8E 0FE94 0FEA2 0FEAA 0FEAE 0FEC2 0FEDA 0FEDE 0FB93 0FECA 0FED2 0FED6 0FEEE 0FEF0 0FEF2', $variants[1])) { // not already included
                    ...
                    $blwf = [];
                    $pstf = [];
                    if (str_contains('dev2 bng2 gur2 gjr2 ory2 tml2 tel2 knd2 mlm2 deva beng guru gujr orya taml telu knda mlym', $st)) { // all INDIC scripts [any/all languages]
                        if (str_contains('deva beng guru gujr orya taml telu knda mlym', $st)) {
                            $is_old_spec = true;
                        } else {
                            $is_old_spec = false;
                        }
                        ...
                        $loclsubs = [];
                        foreach ($volt as $v) {
                            if (str_contains('locl', $v['tag'])) {
                                $key = $v['match'];
                                $key = preg_replace('/[\(\)]*/', '', $key);
                                $sub = $v['replace'];
                                if ($key && 5 == strlen(trim($key)) && $sub) {
                                    $loclsubs[$sub] = $key;
                                }
                            }
                        }
                        ...
                        foreach ($volt as $v) {
                            ...
                            if (str_contains('rphf half pref blwf pstf', $v['tag'])) {
                                if (isset($v['context']) && $v['context'] && 0 == $v['nBacktrack'] && 0 == $v['nLookahead']) {
                                    foreach ($v['rules'] as $vs) {
                                        if (2 == count($vs['match']) && 1 == count($vs['replace'])) {
                                            $sub = $vs['replace'][0];
                                            // If Halant Cons   <pref>, <blwf> and <pstf> in New version only
                                            if (str_contains('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D', $vs['match'][0]) && str_contains('pref blwf pstf', $v['tag']) && !$is_old_spec) {
                                                $key = $vs['match'][1];
                                                $tag = $v['tag'];
                                            ...
                                            elseif (str_contains('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D', $vs['match'][1]) && (str_contains('rphf half', $v['tag']) || (str_contains('pref blwf pstf', $v['tag']) && ($is_old_spec || _OTL_OLD_SPEC_COMPAT_2)))) {
                                                $key = $vs['match'][0];
                                                $tag = $v['tag'];
                                                if (isset($loclsubs[$key])) {
                                                    ${$tag}[$loclsubs[$key]] = $sub;
                                    ...
                                    $key = preg_replace('/[\(\)]*/', '', $key);
                                    $sub = $v['replace'];
                                    if ($key && 11 == strlen(trim($key)) && $sub) {
                                        // If Cons Halant    <rphf> and <half> always
                                        // and <pref>, <blwf> and <pstf> in Old version
                                        // If Halant Cons   <pref>, <blwf> and <pstf> in New version only
                                        if (str_contains('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D', substr($key, 0, 5)) && str_contains('pref blwf pstf', $v['tag']) && !$is_old_spec) {
                                            $key = substr($key, 6, 5);
                                            $tag = $v['tag'];
                                            if (isset($loclsubs[$key])) {
                                                ${$tag}[$loclsubs[$key]] = $sub;
                                            ...
                                            $tmp = &${$tag};
                                            $tmp[hexdec($key)] = hexdec($sub);
                                        } elseif (str_contains('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D', substr($key, 6, 5)) && (str_contains('rphf half', $v['tag']) || (str_contains('pref blwf pstf', $v['tag']) && ($is_old_spec || _OTL_OLD_SPEC_COMPAT_2)))) {
                                            $key = substr($key, 0, 5);
                                            $tag = $v['tag'];
                                            if (isset($loclsubs[$key])) {
                                                ${$tag}[$loclsubs[$key]] = $sub;
                                    ...
                                    for ($lus = 0; $lus < $Lookup[$lup]['SubtableCount']; ++$lus) {
                                        if (count($Lookup[$lup]['Subtable'][$lus]['subs'])) {
                                            foreach ($Lookup[$lup]['Subtable'][$lus]['subs'] as $luss) {
                                                ...
                                                if (!str_contains($inputGlyphs[$seqIndex], $lookupGlyphs[0])) {
                                                    continue;
                                                }
                                                $REPL = implode(' ', $luss['substitute']);
                                                if (str_contains('isol fina fin2 fin3 medi med2 init ', $tag) && 'arab' == $scripttag) {
        ...
        if (isset($this->tables['GPOS'])) {
            ...
            for ($i = 0; $i < $LookupCount; ++$i) {
                for ($c = 0; $c < $Lookup[$i]['SubtableCount']; ++$c) {
                    ...
                    if (7 == $Lookup[$i]['Type'] && 3 == $PosFormat) {
                        $this->skip(4);
                    } elseif (8 == $Lookup[$i]['Type'] && 3 == $PosFormat) {
                        $BacktrackGlyphCount = $this->read_ushort();
                        $this->skip(2 * $BacktrackGlyphCount + 2);
                    }
...
?>';
    ...
    { // mPDF 5.7.1
        ...
        for ($i = 0; $i < $cmapTableCount; ++$i) {
            ...
            $offset = $this->read_ulong();
            $save_pos = $this->_pos;
            if ((3 == $platformID && 10 == $encodingID) || 0 == $platformID) { // Microsoft, Unicode Format 12 table HKCS
                $format = $this->get_ushort($cmap_offset + $offset);
                if (12 == $format) {
                    $unicode_cmap_offset = $cmap_offset + $offset;
    ...
    { // mPDF 5.7.1
        ...
        if ($useOTL) {
            ...
            // Sort CID2GID map into segments of contiguous codes
            ///////////////////////////////////
            unset($charToGlyph[65535], $charToGlyph[0]);
