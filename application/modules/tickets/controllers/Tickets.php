...
class Tickets extends Hosting_Billing
{
    public function __construct()
    {
        ...
        $this->load->model(['Ticket', 'App']);

        App::module_access('menu_tickets');

        $archive = false;
        if (isset($_GET['view'])) {
            if ('archive' == $_GET['view']) {
                $archive = true;
            }
        }
    ...
    public function _ticket_list($archive = null)
    {
        if (User::is_admin()) {
    ...
    public function add(): void
    {
        if ($this->input->post()) {
            ...
            if (false == $this->form_validation->run()) {
                ...
                $additional_fields = [];
                $additional_data = $this->db->where(['deptid' => $_POST['department']])
                    ->get('fields')
                    ->result_array()
                ;
    ...
    public function edit($id = null): void
    {
        if ($this->input->post()) {
            ...
            if (false == $this->form_validation->run()) {
                ...
                Ticket::update_data('tickets', ['id' => $ticket_id], $this->input->post());
            ...
            $this->template
                ->set_layout('users')
                ->build('edit_ticket', $data ?? null)
            ;
        }
    }

    public function quick_edit(): void
    {
        if ($this->input->post()) {
            $ticket_id = $this->input->post('id', true);
        ...
        if ($this->input->post()) {
            ...
            if (false == $this->form_validation->run()) {
                ...
                if ($reply_id = Ticket::save_data('ticketreplies', $insert)) {
                    ...
                    if ('closed' == Ticket::view_by_id($ticket_id)->status) {
                        if ('TRUE' == config_item('notify_ticket_reopened')) {
                            $this->_notify_ticket_reopened($ticket_id);
                        }
                    }

                    Ticket::update_data('tickets', ['id' => $ticket_id], ['status' => 'open']);
    ...
    public function delete($id = null): void
    {
        if ($this->input->post()) {
            ...
            App::delete('ticketreplies', ['ticketid' => $ticket]); //delete ticket replies
            //clear ticket activities
            App::delete('activities', ['module' => 'tickets', 'module_field_id' => $ticket]);
            //delete ticket
            App::delete('tickets', ['id' => $ticket]);
    ...
    public function archive(): void
    {
        ...
        $data = ['archived_t' => $archived];
        Ticket::update_data('tickets', ['id' => $id], $data);
    ...
    public function download_file($ticket = null): void
    {
    ...
    public function status($ticket = null): void
    {
        if (isset($_GET['status'])) {
            $status = $_GET['status'];
            $current_status = Ticket::view_by_id($ticket)->status;

            if ('closed' == $current_status && 'closed' != $status) {
                if ('TRUE' == config_item('notify_ticket_reopened')) {
            ...
            $data = ['status' => $status];
            Ticket::update_data('tickets', ['id' => $ticket], $data);

            if ('closed' == $status && 'closed' != $current_status) {
    ...
    public function _ticket_closed($ticket): void
    {
        if ('TRUE' == config_item('notify_ticket_closed')) {
            ...
            $no_of_replies = App::counter('ticketreplies', ['ticketid' => $ticket]);
    ...
    public function _notify_ticket_reopened($ticket): void
    {
    ...
    public function _upload_attachment($data)
    {
    ...
    public function _admin_tickets($archive = false, $filter_by = null)
    {
        if (null == $filter_by) {
            return Ticket::by_where(['archived_t !=' => '1']);
        }
        ...
        switch ($filter_by) {
            case 'open':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'open']);

            break;

            case 'closed':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'closed']);

            break;

            case 'pending':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'pending']);

            break;

            case 'resolved':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'resolved']);
    ...
    public function _staff_tickets($archive = false, $filter_by = null)
    {
        $staff_department = User::profile_info(User::get_id())->department;
        $dep = json_decode($staff_department, true);

        if (null == $filter_by) {
            ($archive) ? $this->db->where(['archived_t' => '1'])
                    : $this->db->where(['archived_t !=' => '1']);
        ...
        switch ($filter_by) {
            case 'open':
            $this->db->where(['archived_t !=' => '1', 'status' => 'open']);
            if (is_array($dep)) {
                $this->db->where_in('department', $dep);
            } else {
                $this->db->where('department', $staff_department);
            ...
            case 'closed':
            $this->db->where(['archived_t !=' => '1', 'status' => 'closed']);
            if (is_array($dep)) {
                $this->db->where_in('department', $dep);
            } else {
                $this->db->where('department', $staff_department);
            ...
            case 'pending':
            $this->db->where(['archived_t !=' => '1', 'status' => 'pending']);
            if (is_array($dep)) {
                $this->db->where_in('department', $dep);
            } else {
                $this->db->where('department', $staff_department);
            ...
            case 'resolved':
            $this->db->where(['archived_t !=' => '1', 'status' => 'resolved']);
            if (is_array($dep)) {
                $this->db->where_in('department', $dep);
            } else {
                $this->db->where('department', $staff_department);
    ...
    public function _client_tickets($archive = false, $filter_by = null)
    {
        if (null == $filter_by) {
            if ($archive) {
                return Ticket::by_where(['reporter' => User::get_id(), 'archived_t' => '1']);
            }
        ...
        switch ($filter_by) {
            case 'open':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'open', 'reporter' => User::get_id()]);

            break;

            case 'closed':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'closed', 'reporter' => User::get_id()]);

            break;

            case 'pending':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'pending', 'reporter' => User::get_id()]);

            break;

            case 'resolved':
            return Ticket::by_where(['archived_t !=' => '1', 'status' => 'resolved', 'reporter' => User::get_id()]);

            break;

            default:
            return Ticket::by_where(['archived_t !=' => '1', 'reporter' => User::get_id()]);
