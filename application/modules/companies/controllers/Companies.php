...
class Companies extends Hosting_Billing
{
    ...
    public function index(): void
    {
        ...
        $this->template
            ->set_layout('users')
            ->build('companies', $data ?? null)
        ;
    }

    public function view($company = null): void
    {
        $this->load->module('layouts');
        $this->load->library('template');
        $this->template->title(lang('clients').' - '.config_item('company_name'));
        ...
        if ($this->input->post()) {
            ...
            if (false == $this->form_validation->run()) {
                ...
                foreach ($custom_fields as $key => $f) {
                    ...
                    $r = $this->db->where(['client_id' => $company_id, 'meta_key' => $key])->get('formmeta');
                    ...
                    (0 == $r->num_rows()) ? $this->db->insert('formmeta', $data) : $this->db->where(['client_id' => $company_id, 'meta_key' => $cf->row()->name])->update('formmeta', $data);
                }
    ...
    public function update($company = null): void
    {
        if ($this->input->post()) {
            $custom_fields = [];
            foreach ($_POST as $key => &$value) {
    ...
    public function file($id = null): void
    {
        if ('delete' == $this->uri->segment(3)) { // Delete file code
            if ($this->input->post()) {
                ...
                App::delete('files', ['file_id' => $file_id]);
        ...
        } elseif ('add' == $this->uri->segment(3)) { // Adding a file
            if ($this->input->post()) {
                Applib::is_demo();
    ...
    public function send_email($id = null): void
    {
        if ($this->input->post()) {
            $config = [];
            ...
            if ($this->email->send(false)) {
                ...
                $this->session->set_flashdata('message', lang('message_not_sent').'<p>'.$this->email->print_debugger(['headers', 'subject', 'body']).'</p>');
            }
    ...
    public function send_sms($id = null): void
    {
        if ($this->input->post()) {
    ...
    public function send_invoice($user = null): void
    {
    ...
    public function _email_invoice($invoice_id, $message, $subject, $contact): void
    {
        $info = Invoice::view_by_id($invoice_id);
    ...
    public function make_primary(): void
    {
    ...
    public function account(): void
    {
        ...
        $this->load->view('modal/account', $data ?? null);
    }
    ...
    public function import_clients(): void
    {
        $count = 0;
            ...
            if (!empty($file)) {
                ...
                if (!empty($valid)) {
                    ...
                    for ($x = 3; $x <= count($sheetData); ++$x) {
                        if (0 == $this->db->where('co_id', $sheetData[$x]['A'])->where('imported', 1)->get('companies')->num_rows()) {
                            $code = $sheetData[$x]['K'];
                            $string = '[[:<:]]'.$code.'[[:>:]]';
                            $country = $this->db->get_where('countries', ['code REGEXP BINARY ' => $string], false)->row();
