...
class peeker_connect
{
    ...
    public function initialize($init_array): void
    {
        ...
        if ($this->resource) {
            ...
            if ('Mailbox is empty' === $err[0] or 0 === $this->get_message_count()) {
     ...
     *
     * @param mixed $mailbox_name_or_full_server_spec_string
     */
    public function change_to_mailbox($mailbox_name_or_full_server_spec_string): void
    {
        ...
        if (0 === strncmp('{', $mailbox_name_or_full_server_spec_string, 1)) {
    ...
    private function _handle_rejection($err): void
    {
        ...
        if (isset($err[0])) {
            ...
            elseif ('[AUTH] Username and password not accepted.' === $err[0]) {
            ...
            } elseif ('[SYS/PERM] Your account is not enabled for POP access. Please visit your Gmail settings page and enable your account for POP access.' === $err[0]) {
            ...
            } elseif (str_contains('Certificate failure', $err[0])) {
                $reject_reason = 'Using Gmail or other SSL login? Try adding novalidate-cert to the service flags.';
            } elseif ('login allowed only every 15 minutes' === $err[0]) {
        ...
        if (isset($err[1])) {
            if ('POP3 connection broken in response' === $err[1]) {
