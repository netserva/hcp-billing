...
class Invoices extends Hosting_Billing
{
    ...
    public function index(): void
    {
        ...
        $this->template
            ->set_layout('users')
            ->build('invoices', $data ?? null)
        ;
    }

    public function view($invoice_id = null): void
    {
    ...
    public function autoitems(): void
    {
        $query = 'SELECT * FROM (
    ...
    public function add(): void
    {
        ...
        if ($this->input->post()) {
            ...
            if (false == $this->form_validation->run()) {
                ...
                if ('TRUE' == config_item('increment_invoice_number')) {
            ...
            $this->template
                ->set_layout('users')
                ->build('create_invoice', $data ?? null)
            ;
        }
    }
    ...
    public function create($id = null): void
    {
        if ($this->session->userdata('process') && !empty($this->session->userdata('order')->order)) {
            ...
            $data = [
                ...
                'client' => $co_id,
                'tax' => ('TRUE' == config_item('order_tax')) ? config_item('default_tax') : '0.00',
            ...
            if ($invoice_id = App::save_data('invoices', $data)) {
                ...
                foreach ($order as $item) {
                    ...
                    if ('once_off' != $item->renewal
                        && 'one_time_payment' != $item->renewal
                    ...
                    if (isset($i) && 10 == $i->parent && 'total_cost' == $item->renewal) {
                        $description = lang($item->renewal);
                    }

                    if ('promo' == $item->domain) {
                    ...
                    if ($item_id = App::save_data('items', $items)) {
                        if (0 == $i->addon || isset($item->parent) || isset($item->nameservers) || isset($item->domain_only)) {
                            if ('once_off' != $item->renewal
                                    && 'one_time_payment' != $item->renewal) {
                                $username = '-';
                                if (isset($i)) {
                                ...
                                $order = [
                                    ...
                                    'renewal_date' => ('total_cost' == $item->renewal) ? date('Y-m-d') : $renewal_date,
                                ...
                                if ('total_cost' == $item->renewal || 'No' == $i->create_account) {
                                    $order['status_id'] = 2;
                                }
                                ...
                                if ('TRUE' == config_item('affiliates')) {
                                    $aff = affiliate();
                                    if ($aff > 0 && 'none' != $i->commission && $aff != $co_id && !User::is_admin() && !User::is_staff()) {
                                        if ('default' != $i->commission) {
                                            $amount = $i->commission_amount;

                                            if ('percentage' == $i->commission) {
                                        ...
                                        if (null != $i->commission_payout && 'default' != $i->commission_payout) {
                                        ...
                                        App::save_data('referrals', ['affiliate_id' => $aff, 'order_id' => $order_id, 'amount' => $item->price, 'commission' => $amount, 'type' => $payout]);
                                        unset_affiliate();
                                    }
                                }
            ...
            if ($invoice_due <= 0) {
                ...
                if ('TRUE' == config_item('apply_credit') && $client->transaction_value > 0) {
    ...
    {
        if ($this->input->post() && 'true' == $this->input->post('upgrade')) {
            ...
            if ($invoice_id = App::save_data('invoices', $invoice)) {
                ...
                if ($item_id = App::save_data('items', $items)) {
                    ...
                    unset($order['id'], $order['status_id']);
    ...
    public function edit($invoice_id = null): void
    {
            ...
            if ($this->input->post()) {
                ...
                if (false == $this->form_validation->run()) {
                    ...
                    if (Invoice::update($invoice_id, $this->input->post())) {
                        if ('none' != $this->input->post('r_freq')) {
    ...
    public function apply_credit($invoice_id = null): void
    {
        if ($this->input->post()) {
            ...
            if ($payment_id = App::save_data('payments', $data)) {
                ...
                App::update('companies', ['co_id' => $client->co_id], $balance);
    ...
    public function pay($invoice = null): void
    {
        ...
        if ($this->input->post()) {
            ...
            if (false == $this->form_validation->run()) {
                ...
                if ('on' == $this->input->post('attach_slip')) {
                ...
                if (isset($payment_id)) {
                    if (6 == $this->input->post('payment_method')) {
                        $client = Client::view_by_id(Invoice::view_by_id($invoice_id)->client);
                        $credit = $client->transaction_value;
            ...
            $this->template
                ->set_layout('users')
                ->build('pay_invoice', $data ?? null)
            ;
        }
    }

    public function show($invoice_id = null): void
    {
        $data = ['show_client' => 'Yes'];
        Invoice::update($invoice_id, $data);
        Applib::go_to($_SERVER['HTTP_REFERER'], 'success', lang('invoice_visible'));
    }

    public function hide($invoice_id = null): void
    {
        $data = ['show_client' => 'No'];
        Invoice::update($invoice_id, $data);
    ...
    public function cancel($invoice = null): void
    {
        if ($this->input->post()) {
            ...
            App::update('invoices', ['inv_id' => $invoice_id], $data);
    ...
    public function mark_as_paid($invoice = null): void
    {
        if ($this->input->post()) {
            $invoice_id = $this->input->post('invoice');
            $info = Invoice::view_by_id($invoice_id);
    ...
    public function stop_recur($invoice_id = null): void
    {
        if (User::is_client()) {
    ...
    public function transactions($invoice_id = null): void
    {
        ...
        $this->template
            ->set_layout('users')
            ->build('invoice_payments', $data ?? null)
        ;
    }

    public function delete($invoice_id = null): void
    {
        if ($this->input->post()) {
            $invoice = $this->input->post('invoice', true);

            if ($this->db->where('invoice_id', $invoice)->get('orders')->num_rows() > 0) {
                Invoice::update($invoice, ['inv_deleted' => 'Yes']);
            } else {
    ...
    public function add_funds_invoice($company = null): void
    {
    ...
    public function add_funds($company): void
    {
        $data['company'] = $company;
        $this->load->view('modal/add_funds', $data);
    }

    public function remind($invoice = null): void
    {
        if ($this->input->post()) {
    ...
    public function send_invoice($invoice_id = null): void
    {
        if ($this->input->post()) {
    ...
    public function _email_invoice($invoice_id, $message, $subject, $cc): void
    {
        ...
        if ('invoicr' == config_item('pdf_engine')) {
            $invoicehtml = modules::run('fopdf/attach_invoice', $attach);
        }
        if ('mpdf' == config_item('pdf_engine')) {
    ...
    public function pdf($invoice_id = null): void
    {
        if (!User::can_view_invoice(User::get_id(), $invoice_id)) {
            App::access_denied('invoices');
        }
    ...
    public function attach_pdf($invoice_id)
    {
        ...
        return $this->applib->create_pdf($pdf);
    }

    public function _get_clients()
    {
        $sort = ['order_by' => 'date_added', 'order' => 'desc'];

        return Applib::retrieve(Applib::$companies_table, ['co_id !=' => '0']);
    }
