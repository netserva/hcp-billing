...
class Menus extends Hosting_Billing
{
    ...
    public function add_menu(): void
    {
        if (isset($_POST['title'])) {
            ...
            $this->template
                ->set_layout('users')
                ->build('add_menu', $data ?? null)
            ;
        }
    }

    public function edit_menu(): void
    {
        ...
        if ($title) {
            ...
            if ($res) {
                if ('0' == $this->db->where('param', 'menus_'.$id)->where('module', 'Menus')->get('blocks_modules')->num_rows()) {
                    $data = [
                        'name' => $title,
                        'param' => 'menus_'.$id,
                        'type' => 'Module',
     ...
     */
    public function menu($group_id): void
    {
        $menu = $this->Menu->get_menu($group_id);
     ...
     * @return string
     */
    public function generate_list($ul_attr = '')
    {
    ...
    {
        return $this->db->where('id', $id)->update('menu', ['active' => $this->input->post('active')]);
    }
    ...
    public function add_row($id, $parent, $li_attr, $label): void
    {
        $this->temp[$parent][] = ['id' => $id, 'li_attr' => $li_attr, 'label' => $label];
    }
     ...
     */
    public function save_position(): void
    {
    ...
    public function old_save_position(): void
    {
    ...
    public function menus_block($group_id): void
    {
        $object = new stdClass();
        $object->id = $group_id;
        $main_menu = [];
    ...
    private function update_position($parent, $children): void
    {
        $i = 1;
    ...
    private function get_label($row)
    {
        return '<div class="ns-row">'.
                ...
                '</a>'.
                '<a data-rel="tooltip" data-original-title="'.(1 == $row->active ? lang('deactivate') : lang('activate')).'" class="activate-item btn btn-xs btn-'.(0 == $row->active ? 'default' : 'success').'" href="#" data-href="'.base_url().'menus/activate/'.$row->id.'"><i class="fa fa-power-off"></i></a>'.
                '</a>'.
    ...
    private function get_labels($row)
    {
        return '<div class="ns-row">'.
                ...
                '<span class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></span>'.
                '</a>'.
                '<a data-rel="tooltip" data-original-title="'.(1 == $row['active'] ? lang('deactivate') : lang('activate')).'" class="activate-item btn btn-xs btn-'.(0 == $row['active'] ? 'default' : 'success').'" href="#" data-href="'.base_url().'menus/activate/'.$row['active'].'"><i class="fa fa-power-off"></i></a>'.
                '<input type="hidden" name="menu_id" value="'.$row['id'].'">'.
                '</div>'.
                '</div>';
    }
