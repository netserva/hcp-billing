...
$language = App::languages($client->language);
          ...
          if ($v[0] < 5 || (5 == $v[0] && $v[1] < 5)) {
              if ('EUR' == $cur->code) {
                  $invoice->currency = chr(128);
              }
              if ('GBP' == $cur->code) {
                  $invoice->currency = chr(163);
              }
          }
          ...
          if (!empty($state_from)) {
              $country_from = $state_from.', '.$country_from;
          }
          ...
          if (!empty($state_to)) {
              $country_to = $state_to.', '.$country_to;
          }
          ...
          $to = [$client->company_name, $client->company_address, $city_to, $country_to, $to_vat];

          $invoice->setFrom($from);
          $invoice->setTo($to);
          if ('TRUE' == config_item('swap_to_from')) {
              $invoice->flipflop();
          }
          ...
          foreach (Invoice::has_items($inv->inv_id) as $key => $item) {
              if ('TRUE' == config_item('show_invoice_tax')) {
...
//Set badge
if ('TRUE' == config_item('display_invoice_badge')) {
