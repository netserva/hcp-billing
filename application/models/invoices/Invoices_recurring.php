...
class Invoices_Recurring extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT * FROM hd_invoices  WHERE recur_next_date <= date(NOW()) AND (recur_end_date > date(NOW()) OR recur_end_date = '0000-00-00') AND recur_start_date <= date(NOW())")->result();
    }

    public function get_invoice($source_id, $invoice_table)
    {
        return $this->db->where('inv_id', $source_id)->get($invoice_table)->row();
    }

    public function set_next_recur_date($invoice_id): void
    {
     ...
     * @return date
     */
    public function _increment_date($date, $increment)
    {
