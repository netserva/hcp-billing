...
class Ticket extends CI_Model
{
    ...
    public static function by_where($array = null)
    {
        return self::$db->where($array)->order_by('id', 'DESC')->get('tickets')->result();
    }

    // Get ticket information
    public static function view_by_id($ticket)
    {
        return self::$db->where('id', $ticket)->get('tickets')->row();
    }

    // Get ticket replies
    public static function view_replies($id)
    {
    ...
    public static function generate_code()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $next_number = $code + 1;
            if ($next_number < config_item('ticket_start_no')) {
                $next_number = config_item('ticket_start_no');
            }
