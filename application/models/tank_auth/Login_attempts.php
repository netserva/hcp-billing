...
class Login_attempts extends CI_Model
{
     ...
     * @param mixed $login
     */
    public function increase_attempt($ip_address, $login): void
    {
        $this->db->insert($this->table_name, ['ip_address' => $ip_address, 'login' => $login]);
    }
     ...
     */
    public function clear_attempts($ip_address, $login, $expire_period = 86400): void
    {
        $this->db->where(['ip_address' => $ip_address, 'login' => $login]);
