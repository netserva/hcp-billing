...
class App_hooks
{
     ...
     * @var array
     */
    private $ignore_pages = ['/auth/login', '/auth/logout', '/auth/register/', '/auth/forgot_password', '/auth/register', '/auth/resend_activation', 'login', 'logout', 'register', 'set_language'];
    ...
    public function fix_cache(): void
    {
        // Cache control
     ...
     */
    public function prep_redirect(): void
    {
     ...
     */
    public function save_requested(): void
    {
        if (!class_exists('CI_Session')) {
            $this->ci->load->library('session');
        }
