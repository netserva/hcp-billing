...
class Settings
{
    ...
    public function open_form($options)
    {
        $action = '';
    ...
    public function auto_db_to_options($ary, $custom_options = [])
    {
        $options = [];
        ...
        foreach ($ary as $k => $v) {
            ...
            if (is_json($v)) {
                ...
                if (str_contains(strtolower($k), 'date')) {
                    $k = 'date';
                }
            ...
            if (isset($custom_options, $custom_options[$k])) {
                if (is_array($custom_options[$k])) {
                    $elm_options = array_merge($elm_options, $custom_options[$k]);
                }
            }

            if (!(isset($custom_options, $custom_options[$k]) && !is_array($custom_options[$k]) && 'unset' == $custom_options[$k])) {
                $options[] = $elm_options;
            }
        }
    ...
    public function change_pre_built(&$pre_built, $id, $vals_ary): void
    {
    ...
    public function build_form_horizontal($options, $data_source = [])
    {
        $this->_reset_builder();
        ...
        foreach ($options as $elm_options) {
            ...
            if (is_array($this->elm_options)) {
                ...
                switch ($this->func) {
                    ...
                    case 'form_checkbox':
                    case 'form_radio':
                        $link_to_input = ((1 === count($this->elm_options['options'])) && array_key_exists('label', $this->elm_options['options'][0]) && ('' === $this->elm_options['options'][0]['label']));

                        $default_form_control_class = $this->config['default_form_control_class'];
                        if (!array_key_exists('label', $this->elm_options) || ('none' === $this->elm_options['label'])) {
    ...
    public function build_display($options, $data_source = [])
    {
        $this->_reset_builder();
        $this->data_source = (array) $data_source;
        ...
        $this->print_string .= $this->_build_form_open('', ['class' => $this->config['default_form_class']]);
    ...
    public function adv_set_value($field = '', $default = '')
    {
    ...
    public function squish_HTML($html)
    {
    ...
    private function _prep_options(): void
    {
        ...
        $default_value = '';
        if (isset($this->elm_options['name'], $this->data_source[$this->elm_options['name']]) && empty($this->elm_options['value'])) {
        ...
        if ('form_combine' == $this->func) {
            ...
            switch ($this->func) {
                ...
                case 'form_a':
                    ...
                    if (!str_contains($class, $this->config['default_button_classes'])) {
                ...
                case 'form_label':
                    $input_html_string = form_label($this->_make_label($this->elm_options['value']), '', [
                ...
                case 'form_submit':
                    ...
                    $label = $this->_make_label(($this->elm_options['label'] ?? $this->elm_options['id']));

                    unset($this->elm_options['id'], $this->elm_options['label']);
                ...
                case 'form_dropdown':
                    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
                        ...
                        unset($this->elm_options['name'], $this->elm_options['value'], $this->elm_options['options']);
                        ...
                        if (!empty($this->config['default_dropdown_class'])) {
                            ...
                            if (!str_contains($class, $this->config['default_dropdown_class'])) {
                                $class .= ' '.$this->config['default_dropdown_class'];
                            }

                            if (!str_contains($class, $this->config['bootstrap_required_input_class'])) {
                                $class .= ' '.$this->config['bootstrap_required_input_class'];
                            }
                            $this->elm_options['class'] = trim($class);
                        }
    ...
    {
        if (('form_date' === $this->func) && ('' !== $this->config['default_datepicker_class'])) {
            return '<div class="date '.$this->config['default_datepicker_class'].' '.$this->config['default_form_control_class'].'" data-date="'.$this->elm_options['value'].'" data-date-format="'.preg_replace(['/Y/', '/m/', '/d/'], ['yyyy', 'mm', 'dd'], $this->config['default_date_format']).'" data-date-viewmode="years">';
        }
    ...
    {
        $label = '';
        if (isset($this->elm_options['label']) && 'none' == $this->elm_options['label']) {
            return '';
        }
        if (isset($this->elm_options['label'])) {
            $label = $this->elm_options['label'];
        } elseif (isset($this->elm_options['id']) && 'form_submit' != $this->func) {
            $label = $this->_make_label($this->elm_options['id']);
        }

        if ('form_submit' == $this->func) {
            $label = '';
        }

        return form_label($label, $link_to_input_id ? $this->elm_options['name'] : '', [
    ...
    private function _reset_builder(): void
    {
        $this->print_string = '';
        $this->func = $this->config['default_input_type'];
    }
    ...
    private function _recursive_build_json($ary, $offset = 0)
    {
        ...
        foreach ($ary as $k => $v) {
            ...
            if ((is_array($v) || is_object($v)) && !is_string($v)) {
                ...
                $kv_str .= '<div class="'.$offset_class.'"><strong>'.ucwords(strtolower(str_replace(['_', '-'], ' ', $k))).'</strong>'.$innter_str.'</div>';
            } else {
                $kv_str .= '<div class="'.$offset_class.'"><strong>'.ucwords(strtolower(str_replace(['_', '-'], ' ', $k))).'</strong>: '.$v.'</div>';
            }
        }
