...
{
    return [
        'ISPConfig 3 Version' => ['Type' => 'dropdown', 'Options' => '3.0,3.1', 'Default' => '3.0', 'Description' => 'Select the installed version of ISPConfig 3'],
        'Version Details' => ['Description' => 'Select the installed version of ISPConfig 3.'],
        'Template ID' => ['Type' => 'text', 'Size' => '3', 'Description' => '#'],
        'Template ID Details' => ['Description' => 'Locate the template ID number in the ISPConfig 3 menu<br><b>Client » Template » Template ID</b>'],
        'Client Limit' => ['Type' => 'text', 'Size' => '3', 'Default' => '0', 'Description' => '#'],
        'Client Limit Details' => ['Description' => 'If the client is a Reseller, make sure the Client Limit<br>is equal to Max. number of Clients in the selected template.<br><br>Notice: If client is <b>NOT</b> a Reseller, a value of "0" must<br>be entered.'],
        'Domain Tool' => ['Type' => 'yesno', 'Description' => 'Add domain to Domain Tool'],
        'Domain Tool Details' => ['Description' => 'If you are using the Domain Tool in ISPConfig 3, you<br>must tick the check box to properly add the domain<br>for the client.'],
        'Domain Setup' => ['Type' => 'yesno', 'Default' => 'yes', 'Description' => 'Setup Website, DNS & Email for domain'],
        'Domain Setup Details' => ['Description' => 'Tick the check box to add DNS records, default<br>website, and add the domain for email.<br><br>Notice: A DNS template must be properly configured if<br>the Domain Setup option is ticked.'],
        'Website Disk Space' => ['Type' => 'text', 'Size' => '10', 'Description' => 'MB'],
        'Disk Space Details' => ['Description' => 'If the Domain Setup option is ticked, enter the amount<br>of disk space allowed for the default website.'],
        'Website Bandwidth' => ['Type' => 'text', 'Size' => '10', 'Description' => 'MB'],
        'Bandwidth Details' => ['Description' => 'If the Domain Setup option is ticked, enter the amount<br>of monthly bandwidth allowed for the default website.'],
        'DNS Template ID' => ['Type' => 'text', 'Size' => '3', 'Description' => '#'],
        'DNS Template ID Details' => ['Description' => 'If the Domain Setup option is ticked, you must have a<br> <b>DNS » DNS Wizard » DNS Template</b>'],
    ];
}
...
function ispconfig3_CreateAccount($params)
{
    ...
    // Connect to SOAP Server
        ...
        // Retreive the serverid from ispconfig
            ...
            if (1 == count($server['mail_server'])) {
                $defaultmailserver = $server['mail_server'][0]['server_id'];
            } else {
                $rnd = random_int(0, (count($server['mail_server']) - 1));
                $defaultmailserver = $server['mail_server'][$rnd]['server_id'];
            }

            if (1 == count($server['web_server'])) {
                ...
                while ($a <= count($server['web_server'])) {
                    $result = [];
                    $result = $client->sites_web_domain_get($session_id, ['server_id' => $server['web_server'][$b]['server_id'], 'type' => 'vhost']);
