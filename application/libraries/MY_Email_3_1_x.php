 ...
 *
 * @see https://github.com/ivantcholakov/codeigniter-phpmailer
 *
...
class MY_Email extends CI_Email
{
    ...
    public function __construct(array $config = [])
    {
        ...
        self::$default_properties['debug_output'] = (str_contains(PHP_SAPI, 'cli') or defined('STDIN')) ? 'echo' : 'html';
    ...
    public function clear($clear_attachments = false)
    {
        ...
        if ('phpmailer' == $this->mailer_engine) {
    ...
    public function attach($file, $disposition = '', $newname = null, $mime = '', $embedded_image = false)
    {
        ...
        if ('phpmailer' == $this->mailer_engine) {
            if ('' == $mime) {
                ...
                $newname = null === $newname ? basename($file) : $newname;
    ...
    {
        $mailer_engine = str_contains(strtolower($mailer_engine), 'phpmailer') ? 'phpmailer' : 'codeigniter';
    ...
    public function set_smtp_crypto($smtp_crypto = '')
    {
        ...
        if ('tls' != $smtp_crypto && 'ssl' != $smtp_crypto) {
            $smtp_crypto = '';
        }
    ...
    public function set_mailtype($type = 'text')
    {
        ...
        if ('phpmailer' == $this->mailer_engine) {
            $this->phpmailer->isHTML('html' == $type);
        }
    ...
    public function set_encoding($encoding)
    {
        ...
        if (!in_array($encoding, 'phpmailer' == $this->mailer_engine ? self::$encodings_phpmailer : self::$encodings_ci)) {
            $encoding = '8bit';
        }
...
'.$message.'
    ...
    protected function refresh_properties(): void
    {
        foreach (array_keys(self::$default_properties) as $name) {
            $this->{$name} = $this->{$name};
        }
